    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            if(ig.global.gameState !== 'continue') {
                ...
                var party_members_pool = [EntityFemale_sage];
        ...
        update: function() {
            ...
            if(ig.input.state('SHIFT') && this.hoveredUnit !== null && this.units[this.activeUnit].unitType === 'player' && this.units[this.activeUnit].vel.x === 0 && this.units[this.activeUnit].vel.y === 0){
            ...
            if(ig.input.state('rightClick') && !this.menuVisible && this.hoveredUnit === null && this.units[this.activeUnit].unitType === 'player' && this.units[this.activeUnit].vel.x === 0 && this.units[this.activeUnit].vel.y === 0) {
                this.menuVisible = true;
                this.spawnEntity(ig.global.EntityButton_endturn, ig.input.mouse.x + this.screen.x, ig.input.mouse.y + this.screen.y);
                this.spawnEntity(ig.global.EntityButton_suspend, ig.input.mouse.x + this.screen.x, ig.input.mouse.y + this.screen.y + 32);
            }
            ...
            if(!this.scriptCatalog.map0.objective.triggered) {
                ...
                this.scriptCatalog.map0.objective.condition(); // For "defeat all enemies", "sieze the tile"
                //this.scriptCatalog.map0.objective.condition(this.turn_counter, 3); // For "survive for x turns"
            // End battle
            } else {
            ...
            if(this.item_drop !== null) {
                ...
                if(this.units[this.activeUnit].unitType === 'player') {
                    for(i = 0; i < ig.game.units[ig.game.activeUnit].item.length; i++)
                        ig.game.spawnEntity(ig.global.EntityButton_inventory_item, ig.game.screen.x + 32, ig.game.screen.y + (i + 1) * 32, {index: i});
                } else if(this.units[this.activeUnit].unitType === 'enemy') {
                    for(i = 0; i < ig.game.targetedUnit.item.length; i++)
                        ig.game.spawnEntity(ig.global.EntityButton_inventory_item, ig.game.screen.x + 32, ig.game.screen.y + (i + 1) * 32, {index: i});
                }

                ig.game.spawnEntity(ig.global.EntityButton_dropped_item, ig.game.screen.x + 32, ig.game.screen.y + 296);
            }
        },
            ...
            if(this.battleState === 'stats') {
                    ...
                    for(var i = 0; i < this.units.length; i++){
                        if(ig.input.pressed('down')){
                            this.tempUnit = this.units[i + 1];
                        }

                        if(typeof a.statMugshot !== 'undefined')
                            a.statMugshot.draw(15, 25); 
                            
                        this.font20_default.draw(a.name,       250, 30);
                        this.font20_default.draw(a.level,      255,  120);
                        this.font20_default.draw(a.health,     255,  160);
                        this.font20_default.draw(a.health_max, 295, 160);
                        this.font20_default.draw(a.stat.str,   80,  240);
                        // this.font20_default.draw(a.stat.mag,   350, 141);
                        this.font20_default.draw(a.stat.def,   80, 395);
                        this.font20_default.draw(a.stat.res,   80, 435);
                        this.font20_default.draw(a.stat.spd,   80, 355);
                        // luk is now equal to dodge. dodge is the chance of avoiding a critial hit.
                        this.font20_default.draw(a.stat.luk,   480, 160);
                        this.font20_default.draw(a.stat.mov,   240, 240);
                        // Draw derived stats
                        this.font20_default.draw(a.derived_stats.atk, 480, 80);
                        this.font20_default.draw(a.derived_stats.hit_rate, 480, 120);
                        this.font20_default.draw('1 ~ ' + a.atk_dist, 565, 40);
                        this.font20_default.draw(a.derived_stats.crit_rate, 580, 80);
                        this.font20_default.draw(a.derived_stats.evade, 580, 120);
                        this.font20_default.draw(a.derived_stats.attack_speed, 580, 160);

                        for(var i = 0; i < a.item.length; i++){
                            if(a.item[i] !== null){
                                a.item[0].icon.draw(380, 250 + i * 40);
                                this.font20_default.draw(a.item[i].name, 400, 250 + i * 40);
                                this.font20_default.draw(a.item_uses[i] + '/' +  a.item[i].uses, 540, 250 + i * 40);
                            }
                        }
    ...
    ig.GameOver = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            if(this.displayExtra)
                this.font16_default.draw('Click to return to start screen', ig.system.width / 2, 440, ig.Font.ALIGN.CENTER);

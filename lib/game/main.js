    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            if(ig.global.gameState !== 'continue') {
                ...
                var party_members_pool = [EntityArcher];
        ...
        update: function() {
            ...
            //Map objective for seize
            if(!this.scriptCatalog.map1.objective.triggered){
                this.scriptCatalog.map1.objective.condition();
            }else{
                unitFocus.pos = ig.global.alignToGrid(unitFocus.pos.x, unitFocus.pos.y);
                unitFocus.vel = {x: 0, y: 0};                
            }
            ...
            if((this.units[this.activeUnit].selectedWeapon !== null && this.units[this.activeUnit].isEquipping) ||
               ...
               var u = this.units[this.activeUnit].unitType === 'player' ? this.units[this.activeUnit] : this.units[this.activeUnit].unitType === 'enemy' ? this.targetedUnit : null;
                ...
                if(typeof u !== null) {
                    ...
                    this.font20_black.draw('Affinity: ' + u.selectedWeapon.affinity, 510, 370, ig.Font.ALIGN.CENTER);

                    // Draw stats for new weapon
                    this.font20_black.draw('Atk:', 410, 403);
                    if(atkChange > u.derived_stats.atk)
                        this.font16_green.draw(u.derived_stats.atk + '->' + atkChange, 450, 404);
                    else if(atkChange < u.derived_stats.atk)
                        this.font16_red.draw(u.derived_stats.atk + '->' + atkChange, 450, 404);
                    else
                        this.font16_default.draw(u.derived_stats.atk, 450, 404);

                    this.font20_black.draw('Hit:', 410, 437);
                    if(hitChange > u.derived_stats.hit_rate)
                        this.font16_green.draw(u.derived_stats.hit_rate + '->' + hitChange, 450, 438);
                    else if(hitChange < u.derived_stats.hit_rate)
                        this.font16_red.draw(u.derived_stats.hit_rate + '->' + hitChange, 450, 438);
                    else
                        this.font16_default.draw(u.derived_stats.hit_rate, 450, 438);

                    this.font20_black.draw('Crit:', 530, 403);
                    if(critChange > u.derived_stats.crit_rate)
                        this.font16_green.draw(u.derived_stats.crit_rate + '->' + critChange, 575, 404);
                    else if(critChange < u.derived_stats.crit_rate)
                        this.font16_red.draw(u.derived_stats.crit_rate + '->' + critChange, 575, 404);
                    else
                        this.font16_default.draw(u.derived_stats.crit_rate, 575, 404);

...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.music.add('media/sounds/bgm/czhang/FireEmblem_-_Companions_CZ_Rearrange.*', 'companions');
            ig.music.add('media/sounds/bgm/awakening.*',           'awakening'          );
            ig.music.add('media/sounds/bgm/battle.*',              'battle'             ); // Track name: Strange Encounter
            ig.music.add('media/sounds/bgm/inescapeable_fate.*',   'inescapeable_fate'  );
            ig.music.add('media/sounds/bgm/winds_across_plains.*', 'winds_across_plains');
        } // End init method
    }); // End ig.BaseGame
            ...
            if(this.units[this.activeUnit] !== this.targetedUnit) {
                ...
                // Initiate battle animation sequence ------------------------------------------------------------
                    ...
                    if(this.attackQueueIndex === 0 && this.atkAnimTimer.delta() > 1) {
                        if(this.units[this.activeUnit].hasEquippedItem === true) {
                    ...
                    // Counterattack -------------------------------------------
                        ...
                        if(this.targetedUnit.hasEquippedItem === true) {
                    ...
                    // Second attack -------------------------------------------
                        ...
                        if(this.attackQueue[2] === this.attackQueue[0] && this.units[this.activeUnit].hasEquippedItem === true) {
                            // Active unit has not started their attack animation yet
                            if(this.attackQueue[0].currentAnim === this.attackQueue[0].anims.idle) {
                                // Compute if active unit triggers critical hit on target unit
                                if(this.computeBattleCritChance(this.units[this.activeUnit], this.targetedUnit) > Math.random() * 100) {
                        ...
                        } else if(this.attackQueue[2] === this.attackQueue[1] && this.targetedUnit.hasEquippedItem === true) {
                            // Target unit has not started their attack animation yet
                            if(this.attackQueue[1].currentAnim === this.attackQueue[1].anims.idle) {
                                // Compute if target unit triggers critical hit on active unit
                                if(this.computeBattleCritChance(this.targetedUnit, this.units[this.activeUnit]) > Math.random() * 100) {
        ...
        */
        ...
        draw: function() {
            ...
            if(this.battleState === 'objective'){
                ...
                this.font30.default.draw(main_player_name, 375, 260);

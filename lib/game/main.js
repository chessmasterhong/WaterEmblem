...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        menu: new ig.Image('media/gui/menu.png'),
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
                    ...
                    // Player unit is trading with target unit
                        ...
                        if(typeof ig.game.getEntityByName('btn_item_a0') === 'undefined') {
                            for(var i = 0; i < this.units[this.activeUnit].item.length; i++) {
                                ig.game.spawnEntity(Button, this.screen.x + 200, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_a' + i,
                                    index: i,
                        ...
                        if(typeof ig.game.getEntityByName('btn_item_t0') === 'undefined') {
                            for(var i = 0; i < this.targetedUnit.item.length; i++) {
                                ig.game.spawnEntity(Button, this.screen.x + 450, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_t' + i,
                                    index: i,
        ...
        draw: function() {
            ...
            if(ig.game.battleState !== 'battle_start'){
                this.menu.draw(23,20);
            }

            if(ig.game.battleState === 'itemMenu'){
                this.menu.draw(194, 20);
            }

            if(ig.game.battleState === 'equipMenu'){
                this.menu.draw(194, 20);
                this.equipMenu.draw(317, 20);
            }
            ...
            if(typeof ig.game.getEntityByName('btn_item_a0') && ig.game.battleState === 'trading'){
                ...
                a.statMugshot.draw(130, 0);
                t.statMugshot.draw(382, 0);
                this.tradeMenu.draw(176, 200);
                this.tradeMenu.draw(430, 200);
            ...
            if(this.units[this.activeUnit].selectedWeapon !== null && this.units[this.activeUnit].isEquipping) {
                ...
                this.font.draw(a.derived_stats.atk + "->" + atkChange, 455, 405);
                this.font.draw(a.derived_stats.hit_rate + "->" + hitChange, 440, 440);
                this.font.draw(a.derived_stats.crit_rate + "->" + critChange, 555, 405);
                this.font.draw(evadeChange, 570, 440);

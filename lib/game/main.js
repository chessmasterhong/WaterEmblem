    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            if(ig.global.gameState !== 'continue') {
                ...
                var party_temp = [];
                var party_temp_pos = [];
                var party_members_pool = [ig.global.EntityFemale_sage, ig.global.EntitySword_master, ig.global.EntityWyvern_lord];
                ...
                for(var p = 1; p <= party_members_pool.length; p++) {
                    ...
                    party_temp[p] = this.spawnEntity(party_members_pool[p - 1], party_member_pos.x, party_member_pos.y);
                }

                for(var i = 0; i < ig.global.party.length; i++) {
                    // Temporarily store unit position for this map
                    party_temp_pos[i] = party_temp[i].pos;

                    // Overwrite newly spawned party (party_temp) with stored game properties for party units
                    for(var prop in ig.global.party[i]) {
                        party_temp[i][prop] = ig.global.party[i][prop];
                    }

                    // Overwrite unit's position from stored game properties with proper position for this map
                    party_temp[i].pos = party_temp_pos[i];
                }
        ...
        update: function() {
                ...
                // Initiate battle animation sequence ------------------------------------------------------------
                    ...
                    // Post-battle computations --------------------------------
                        ...
                        if(this.units[this.activeUnit].unitType === 'player' && this.targetedUnit.unitType === 'enemy' && this.battleAnimStats.targetedUnit.health <= 0) {
                            ...
                            if(slot !== null) {
                                this.item_drop = this.generateRandomItem();
                                this.item_drop_uses = this.item_drop.uses;
                                this.units[this.activeUnit].item[slot] = this.item_drop;
                                this.units[this.activeUnit].item_uses[slot] = this.item_drop.uses;
                                console.log(this.units[this.activeUnit].name + ' obtained ' + this.item_drop.name + '.');
                            } else {
                                this.item_drop = this.generateRandomItem();
                                this.item_drop_uses = this.item_drop.uses;
                        ...
                        // Player unit is defending unit; player unit defeated enemy unit (via counter attack)
                            ...
                            if(slot !== null) {
                                this.item_drop = this.generateRandomItem();
                                this.item_drop_uses = this.item_drop.uses;
                                this.targetedUnit.item[slot] = this.item_drop;
                                this.targetedUnit.item_uses[slot] = this.item_drop.uses;
                                console.log(this.targetedUnit.name + ' obtained ' + this.item_drop.name + '.');
                            } else {
                        ...
                        if(this.units[this.activeUnit].unitType === 'player') {
                            ...
                            if(this.targetedUnit.unitType === 'enemy') {
                                ...
                                if(this.battleAnimStats.targetedUnit.health <= 0) {
                                    this.exp_old = this.units[this.activeUnit].exp_curr;
                                    this.exp_gain = this.computeExperience(this.units[this.activeUnit], this.targetedUnit, true);
                                    this.units[this.activeUnit].exp_curr += this.exp_gain;
                                    console.log(this.units[this.activeUnit].name + ' gained ' + this.exp_gain + ' exp and now has ' + this.units[this.activeUnit].exp_curr + ' exp for current level ' + this.units[this.activeUnit].level + '.');
                                // Enemy unit is still alive
                                    ...
                                    if(this.battleAnimStats.targetedUnit.health !== this.targetedUnit.health) {
                                        this.exp_old = this.units[this.activeUnit].exp_curr;
                                        this.exp_gain = this.computeExperience(this.units[this.activeUnit], this.targetedUnit, false);
                                        this.units[this.activeUnit].exp_curr += this.exp_gain;
                                        console.log(this.units[this.activeUnit].name + ' gained ' + this.exp_gain + ' exp and now has ' + this.units[this.activeUnit].exp_curr + ' exp for current level ' + this.units[this.activeUnit].level + '.');
                        ...
                        // Enemy unit is the attacking unit
                            ...
                            if(this.targetedUnit.unitType === 'player') {
                                ...
                                if(this.battleAnimStats.activeUnit.health <= 0) {
                                    this.exp_old = this.targetedUnit.exp_curr;
                                    this.exp_gain = this.computeExperience(this.targetedUnit, this.units[this.activeUnit], true);
                                    this.targetedUnit.exp_curr += this.exp_gain;
                                    console.log(this.targetedUnit.name + ' gained ' + this.exp_gain + ' exp and now has ' + this.units[this.activeUnit].exp_curr + ' exp for current level ' + this.units[this.activeUnit].level + '.');
            ...
            if(this.battleState === 'exp' && this.expTimer.delta() > 1.5 &&
                (
                    (this.units[this.activeUnit].unitType === 'player' && this.units[this.activeUnit].leveledUp === true) ||
            ...
            if(ig.input.pressed('SHIFT') && this.hoveredUnit !== null && this.units[this.activeUnit].unitType === 'player' && this.units[this.activeUnit].vel.x === 0 && this.units[this.activeUnit].vel.y === 0 && !this.menuVisible) {
        ...
        fadeInFromBattleAnim: function() {
            this.screenFadeInFromBattleAnim = new ig.ScreenFader({
                ...
                callback: function() {
                    ...
                    if((this.item_drop !== null || this.units[this.activeUnit].leveledUp || this.targetedUnit.leveledUp) && (this.units[this.activeUnit].unitType === 'player' || this.units[this.activeUnit].unitType === 'enemy')) {
        ...
        fadeOutAfterEffect: function() {
            this.screenFadeOutAfterEffect = new ig.ScreenFader({
                ...
                callback: function() {
                    ...
                    if(
                        (this.units[this.activeUnit].leveledUp === true && this.units[this.activeUnit].unitType === 'player') ||
                        (this.targetedUnit && this.targetedUnit.unitType === 'player' && this.targetedUnit.leveledUp === true)
                    ){
                        this.levelUpTimer.reset();
                        this.levelUpMessageTimer.reset();
                    ...
                    if(this.battleState === null && (this.units[this.activeUnit].unitType === 'player' || (this.targetedUnit && this.targetedUnit.unitType === 'player'))) {
                        this.expTimer.reset();
        ...
        computeExperience: function(attacker, target, killed) {
            ...
            return killed ? 100 : Math.floor((32 + attacker.level + target.level ) / 4);
        },
        ...
        draw: function() {
            ...
            if(this.battleState === 'exp') {
                if(this.expTimer.delta() < 1) {
                    ...
                    var expNext = 1;
                    var expCurr = this.exp_old + this.exp_gain * ((this.exp_time + (this.expTimer.delta() < 0 ? this.expTimer.delta() : 0)) / this.exp_time);
            ...
            if(this.units[this.activeUnit].unitType === 'player' && this.battleState !== 'battle_start' && this.hoveredTerrain !== null && this.battleState !== 'stats' && !this.menuVisible && this.battleState !== 'trade' && this.battleState !== 'trading' && typeof this.hoveredTerrain !== 'undefined') {
                ...
                if(this.hoveredUnit !== null && this.attackTargets.length > 0) {
                    for(var i = 0; i < this.attackTargets.length; i++) {
                        if(this.hoveredUnit === this.attackTargets[i].unit) {
                            ...
                            this.font16.default.draw(this.computeBattleNormalHitChance(this.units[this.activeUnit], this.hoveredUnit) + this.weaponTriangle(this.units[this.activeUnit], this.hoveredUnit) - hoveredUnit_terrain.avoid,
                                                                                                                                       600, 118                      );
                            ...
                            this.font16.default.draw(this.computeBattleNormalHitChance(this.hoveredUnit, this.units[this.activeUnit]) + this.weaponTriangle(this.hoveredUnit, this.units[this.activeUnit]) - activeUnit_terrain.avoid,
    ...
    */
            ...
            this.screenFadeIn = new ig.ScreenFader({
                ...
                callback: function() {
                    if(ig.global.objective_triggered === true) {
                        ...
                        if(ig.global.currentLevel < this.dialogCatalog.length) {
                            ig.system.setGame(ig.CutScene);
                        } else {
    ...
    ig.GameCredits = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(
                (this.credits_roll !== null && this.credits_roll.maxY >= 0 && this.credits_roll.pos.y <= -this.credits_roll.maxY) ||
                (ig.input.pressed('leftClick') || ig.input.pressed('rightClick'))
            ) {

...
.defines(function() {
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
                    ...
                    // Player unit is trading with target unit
                        ...
                        if(typeof this.getEntityByName('btn_item_a0') === 'undefined') {
                            for(var i = 0; i < this.units[this.activeUnit].item.length; i++) {
                                this.spawnEntity(Button, this.screen.x + 160, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_a' + i,
                                    index: i,
                                    iconLeft: this.units[this.activeUnit].item[i] !== null ? this.units[this.activeUnit].item[i].icon : null,
                                    text: [this.units[this.activeUnit].item[i] !== null ? this.units[this.activeUnit].item[i].name : '----'],
                                    textRight: [this.units[this.activeUnit].item[i] !== null ? this.units[this.activeUnit].item_uses[i] : ''],
                        ...
                        if(typeof this.getEntityByName('btn_item_t0') === 'undefined') {
                            for(var i = 0; i < this.targetedUnit.item.length; i++) {
                                this.spawnEntity(Button, this.screen.x + 400, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_t' + i,
                                    index: i,
                                    iconLeft: this.targetedUnit.item[i] !== null ? this.targetedUnit.item[i].icon : null,
                                    text: [this.targetedUnit.item[i] !== null ? this.targetedUnit.item[i].name : '----'],
                                    textRight: [this.targetedUnit.item[i] !== null ? this.targetedUnit.item_uses[i] : ''],
                                    animSheet: new ig.AnimationSheet('media/gui/bignewMenu.png', 200, 32),
                                    size: {x: 200, y: 32},
                        ...
                        if(this.units[this.activeUnit].selectedItemIndex !== null && this.targetedUnit.selectedItemIndex !== null) {
                            ...
                            this.targetedUnit.item = trade.inventory.t;
                            this.targetedUnit.item_uses = trade.item_uses.t;
        ...
        swapItems: function(inventory, item_uses, index_1, index_2) {
            var inventory_temp = inventory[index_1];
            inventory[index_1] = inventory[index_2];
            inventory[index_2] = inventory_temp;

            var item_uses_temp = item_uses[index_1];
            item_uses[index_1] = item_uses[index_2];
            item_uses[index_2] = item_uses_temp;

            console.log('Swapping Items');
            return {inventory: inventory, item_uses: item_uses};
        },

        // Trading items between two unit's unventory
        tradeItems: function(inventory_activeUnit, inventory_targetUnit, item_uses_activeUnit, item_uses_targetUnit, index_1, index_2) {
            // Swap inventory items
            var inventory_temp = inventory_activeUnit[index_1];
            inventory_activeUnit[index_1] = inventory_targetUnit[index_2];
            inventory_targetUnit[index_2] = inventory_temp;

            // Swap item usage
            var item_uses_temp = item_uses_activeUnit[index_1];
            item_uses_activeUnit[index_1] = item_uses_targetUnit[index_2];
            item_uses_targetUnit[index_2] = item_uses_temp;

            return {
                inventory: {a: inventory_activeUnit, t: inventory_targetUnit},
                item_uses: {a: item_uses_activeUnit, t: item_uses_targetUnit}
            };
        },

        // Method to reset all of the values inside the mod property.
        clearMod: function() {
            this.units[this.activeUnit].mod.atk   = 0;
            this.units[this.activeUnit].mod.hit   = 0;
            this.units[this.activeUnit].mod.crit  = 0;
            this.units[this.activeUnit].mod.wt    = 0;
            this.units[this.activeUnit].mod.evade = 0;
            return this.units[this.activeUnit].mod;
        },

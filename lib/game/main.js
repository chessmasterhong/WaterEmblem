...
.defines(function() {
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            if(ig.input.state('items') && this.units[this.activeUnit].unitType === 'player' && !this.menuVisible) {
                this.menuVisible = true;
                ...
                ig.game.spawnEntity(Button, ig.game.screen.x + 75, ig.game.screen.y + 65, {
                    ...
                    pressedDown: function() {
                        ...
                        // Check if first child buttons exist (check item button #0). If not spawn them, else do nothing.
                        if(typeof ig.game.getEntityByName('btn_item_0') === 'undefined') {
                            ...
                            // Spawn the items
                            for(var i = 0; i < ig.game.units[ig.game.activeUnit].items.length; i++) {
                                //console.log(ig.game.units[ig.game.activeUnit].items[i]);
                                ig.game.spawnEntity(Button, ig.game.screen.x + 200, ig.game.screen.y + (i + 1) * 65, {
                                    name: 'btn_item_' + i,
                                    font: new ig.Font('media/arial.font_14.png'),
                                    text: [ig.game.units[ig.game.activeUnit].items[i]],
                                    textPos: {x: 37, y: 8},
                                    textAlign: ig.Font.ALIGN.CENTER,
                                    size: {x: 75, y: 23},
                                    animSheet: new ig.AnimationSheet('media/button.png', 75, 23),
                                            ...
                                            ig.game.spawnEntity(Button, ig.game.screen.x + 300, ig.game.screen.y + 200, {
                                                ...
                                                pressedDown: function() {
                                                    ...
                                                    if(ig.game.selectedItem == ig.game.units[ig.game.activeUnit].items[0]) {
                                                        ...
                                                        var buttons = ig.game.getEntityByName('btn_item_0');
                                                        var buttons2 = ig.game.getEntityByName('btn_item_1');
                                                        buttons.kill();
                                                        buttons2.kill();
                                                        this.kill();
                                                    } else {
                                                        ...
                                                        // Experimental swap
                                                        ig.game.units[ig.game.activeUnit].items = ig.game.swapItems(ig.game.units[ig.game.activeUnit].items, 0, 1);
                                                        console.log(ig.game.units[ig.game.activeUnit].items);

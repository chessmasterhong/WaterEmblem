...
.defines(function() {
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        // Maps
        levels: [
            ig.global.LevelBig_castle,
            ig.global.LevelBattlefield
        ],
        ...
        recomputeStats: function(entity) {
            ...
            var equippedItem;
            if(entity.item[0] !== null && entity.item[0].type === 'equip' && this.isInArray(entity.item[0].affinity, entity)) {
                equippedItem = {
        ...
        /**
         * boolean isInArray(string value, object entity)
         * -----
         * Checks if the selected weapon exists within entity property "validWeapon"
        */
        isInArray: function(value, entity){
            for(var prop in entity.validWeapon){
                if(entity.validWeapon.hasOwnProperty(prop)){
                    if(entity.validWeapon[prop] === value){
                        return true;
                    }
                }
            }
                    ...
                    // because the `i` never gets incremented.
                        ...
                        for(var i = 0; i < a.item.length; i++){
                            if(a.item[i] !== null){
                                ...
                                this.font20.default.draw(a.item_uses[i] + '/' +  a.item[i].uses, 540, 250 + i * 40);
                            }
                        }
    ...
    ig.CutScenes = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            if(this.dialog_index < this.dialogCatalog.map1.dialog.length)
                this.font20.default.draw(this.dialogCatalog.map1.dialog[this.dialog_index], 0, 360);
        }
    });

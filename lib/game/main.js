...
ig.global.encounter = {zone: 0, spawn_min: 3, spawn_max: 3};
        ...
        update: function() {
            ...
            if(this.units[this.activeUnit] !== this.targetedUnit) {
                if(this.battleState === 'attack') {
                    ...
                    var a_name = this.units[this.activeUnit].name;
                    var a_unitType = this.units[this.activeUnit].unitType;
                    var a_health = this.units[this.activeUnit].health;
                    var a_health_max = this.units[this.activeUnit].health_max;
                    var a_stat = this.units[this.activeUnit].stat;
                    var a_derived_stats = this.units[this.activeUnit].derived_stats;
                    var a_mod = this.units[this.activeUnit].mod;
                    var a_item = this.units[this.activeUnit].item;

                    this.battleAnimStats.activeUnit = {
                        name:          a_name,
                        unitType:      a_unitType,
                        health:        a_health,
                        health_max:    a_health_max,
                        stat:          a_stat,
                        derived_stats: a_derived_stats,
                        mod:           a_mod,
                        item:          a_item
                    };

                    var t_name = this.targetedUnit.name;
                    var t_unitType = this.targetedUnit.unitType;
                    var t_health = this.targetedUnit.health;
                    var t_health_max = this.targetedUnit.health_max;
                    var t_stat = this.targetedUnit.stat;
                    var t_derived_stats = this.targetedUnit.derived_stats;
                    var t_mod = this.targetedUnit.mod;
                    var t_item = this.targetedUnit.item;

                    this.battleAnimStats.targetedUnit = {
                        name:          t_name,
                        unitType:      t_unitType,
                        health:        t_health,
                        health_max:    t_health_max,
                        stat:          t_stat,
                        derived_stats: t_derived_stats,
                        mod:           t_mod,
                        item:          t_item
                    };

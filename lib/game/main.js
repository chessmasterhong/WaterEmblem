...
.defines(function() {
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
                ...
                if(this.battleState === 'attack') {
                    ...
                    // Player unit is trading with target unit
                        ...
                        if(typeof ig.game.getEntityByName('btn_item_a0') === 'undefined') {
                            for(var i = 0; i < this.units[this.activeUnit].item.length; i++) {
                                ig.game.spawnEntity(Button, this.screen.x + 160, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_a' + i,
                                    index: i,
                                    iconLeft: ig.game.units[ig.game.activeUnit].item[i] !== null ? ig.game.units[ig.game.activeUnit].item[i].icon : null,
                                    text: [ig.game.units[ig.game.activeUnit].item[i] !== null ? ig.game.units[ig.game.activeUnit].item[i].name : '----'],
                                    textRight: [ig.game.units[ig.game.activeUnit].item[i] !== null ? ig.game.units[ig.game.activeUnit].item_uses[i] : ''],
                        ...
                        if(typeof ig.game.getEntityByName('btn_item_t0') === 'undefined') {
                            for(var i = 0; i < this.targetedUnit.item.length; i++) {
                                ig.game.spawnEntity(Button, this.screen.x + 400, this.screen.y + (i + 1) * 32 + 200, {
                                    name: 'btn_item_t' + i,
                                    index: i,
                                    iconLeft: ig.game.targetedUnit.item[i] !== null ? ig.game.targetedUnit.item[i].icon : null,
                                    text: [ig.game.targetedUnit.item[i] !== null ? ig.game.targetedUnit.item[i].name : '----'],
                                    textRight: [ig.game.targetedUnit.item[i] !== null ? ig.game.targetedUnit.item_uses[i] : ''],
                                    animSheet: new ig.AnimationSheet('media/gui/bignewMenu.png', 200, 32),
                                    size: {x: 200, y: 32},
                        ...
                        if(this.units[this.activeUnit].selectedItemIndex !== null && this.targetedUnit.selectedItemIndex !== null) {
                            ...
                            var trade = this.tradeItems(
                                this.units[this.activeUnit].item,
                                this.targetedUnit.item,
                                this.units[this.activeUnit].item_uses,
                                this.targetedUnit.item_uses,
                                this.units[this.activeUnit].selectedItemIndex,
                                this.targetedUnit.selectedItemIndex
                            );

                            this.units[this.activeUnit].item = trade.inventory.a;
                            this.units[this.activeUnit].item_uses = trade.item_uses.a;
                            this.targetedUnit.item = trade.inventory.b;
                            this.targetedUnit.item_uses = trade.item_uses.b;
        ...
        tradeItems: function(inventory_a, inventory_b, item_uses_a, item_uses_b, index_a, index_b) {
            // Swap inventory items
            var inventory_temp = inventory_a[index_a];
            inventory_a[index_a] = inventory_b[index_b];
            inventory_b[index_b] = inventory_temp;

            // Swap item usage
            var item_uses_temp = item_uses_a[index_a];
            item_uses_a[index_a] = item_uses_b[index_b];
            item_uses_b[index_b] = item_uses_temp;

            return {
                inventory: {a: inventory_a, b: inventory_b},
                item_uses: {a: item_uses_a, b: item_uses_b}
            };
        },

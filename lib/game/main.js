    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(this.units[this.activeUnit].turnUsed) {
                //Increase turn counter whenever main player moves
                if(this.units[this.activeUnit] instanceof EntityHero){
                    this.turn_counter++;
                    console.log(this.units[this.activeUnit].name + " had " + this.turn_counter + " turn(s). ");
                }
            ...
            if(!this.scriptCatalog.map1.objective.triggered){
                //check end map condition
                this.scriptCatalog.map1.objective.condition(this.turn_counter);
            //End current map
            }else{
                // Stop player's movement, wait for fade transition to finish, and leave battlefield
                unitFocus.pos = ig.global.alignToGrid(unitFocus.pos.x, unitFocus.pos.y);

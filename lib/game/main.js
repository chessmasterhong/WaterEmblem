    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        recomputeStats: function(entity) {
            ...
            entity.derived_stats.atk = entity.stat.str + entity.item[0].atk;
            // Compute hit_rate = base skl / 2 + base luk / 2 + weapon hit rate
            entity.derived_stats.hit_rate = entity.stat.skl / 2 + entity.stat.luk / 2 + entity.item[0].hit;
            // Compute crit rate = base skl / 2 + weapon crit rate
            entity.derived_stats.crit_rate = entity.stat.skl / 2 + entity.item[0].crit;
            // Compute attack speed of entity
            if(entity.stat.wt > entity.item[0].wt) {
                ...
                entity.derived_stats.attack_speed = entity.stat.spd - (entity.item[0].wt - entity.stat.wt);
            }
            // If the attack speed was computed to be negative, make it zero.
            if(entity.derived_stats.attack_speed < 0)
                entity.derived_stats.attack_speed = 0;
        ...
        draw: function() {
            ...
            if(this.battleAnimStage === 'attack' && this.units[this.activeUnit] === this.targetedUnit) {
                if(this.hoveredUnit !== null && this.targets.length > 0) {
                    for(var i = 0; i < this.targets.length; i++) {
                        if(this.hoveredUnit === this.targets[i]) {

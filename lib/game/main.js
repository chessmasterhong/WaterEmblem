        ...
        init: function() {
            ...
            for(var p = 1; p <= party_members_pool.length; p++) {
                ig.global.party[p] = this.spawnEntity(
                    party_members_pool[p - 1],
                    Math.floor((ig.global.party[0].pos.x + 2 * ig.global.tilesize) / ig.global.tilesize) * ig.global.tilesize,
                    Math.floor((ig.global.party[0].pos.y - (party_members_pool.length + party_members_pool.length * ig.global.tilesize) + p * 2 * ig.global.tilesize) / ig.global.tilesize) * ig.global.tilesize
                );
            }

...
.requires(
    ...
    'game.entities.players.monk', // Player's party members
...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.input.bind( ig.KEY.MOUSE2,     'click' );   // Right Click: Interacts with "menus" currently.
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            this.camera.trap.size = {x: ig.system.width / 10, y: ig.system.height / 3};
        ...
        draw: function() {
            ...
            // Draw a sample stat screen for the active unit.
            // In the future, we want to spawn the stat screen on a key press.
            // i.e, spawn the stat screen if a player is right clicked.

                if(ig.input.state('menu')){
                    if(this.units[this.activeUnit].unitType === "player"){
                        this.statScreen.draw(0, 0);
                        this.font.draw(ig.game.units[ig.game.activeUnit].name,   150, 258, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].level,   80, 377, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].health_max,   140, 430, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].health,   100, 430, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.atk, 350, 95, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.mag, 350, 145, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.def, 350, 190, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.res, 350, 235, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.spd, 350, 285, ig.Font.ALIGN.CENTER);
                        this.font.draw(ig.game.units[ig.game.activeUnit].stat.luk, 350, 335, ig.Font.ALIGN.CENTER);
                    }

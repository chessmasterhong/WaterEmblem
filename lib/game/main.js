...
.defines(function() {
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        scriptCatalog: new ScriptCatalog(),
        ...
        init: function() {
            ...
            if(ig.global.gameState !== 'continue') {
                ...
                var enemy_spawner = this.getEntitiesByType(EntityEnemy_spawner);
                if(enemy_spawner.length > 0)
                    for(var e = 0; e < enemy_spawner.length; e++)
                ...
                ig.global.party[0] = this.spawnEntity(
                    EntityHero,
                    480, //Math.floor(mapsize.x * 0.75 / ig.global.tilesize) * ig.global.tilesize,
                    800, //Math.floor(mapsize.y * 0.5  / ig.global.tilesize) * ig.global.tilesize,
                    {name: ig.global.main_player_name}
                );

                // Spawn party members (non-main character)
                var party_members_pool = [EntityAdvancedHero];
                for(var p = 1; p <= party_members_pool.length; p++) {
                    ...
                    var party_member_pos = ig.global.alignToGrid(
                        ...
                        ig.global.party[0].pos.y + 2 * ig.global.tilesize
                    );
        ...
        update: function() {
            ...
            if(this.units[this.activeUnit].turnUsed) {
                ...
                if(this.units[this.activeUnit] instanceof EntityHero) {
                    this.turn_counter++;
                ...
                for(var u = 0; u < this.units.length; u++) {
                    ...
                    if(this.units[u] instanceof EntityHero && !this.units[u]._killed) {
            ...
            if(this.units[this.activeUnit] !== this.targetedUnit) {
                if(this.battleState === 'attack') {
                    ...
                    if(this.battleState === 'trade' && this.units[this.activeUnit].unitType === 'player' && this.targetedUnit.unitType === 'player') {
                        // Find unit from list of available targets
                        for(var t = 0; t < this.tradeTargets.length; t++) {
                ...
                // Initiate battle animation sequence ------------------------------------------------------------
                    ...
                    // Post-battle computations --------------------------------
                        ...
                        if(this.units[this.activeUnit].unitType === 'player' && this.targetedUnit.unitType === 'enemy' && this.battleAnimStats.targetedUnit.health <= 0) {
                            var slot = this.getFreeSlot(this.units[this.activeUnit]);
                            if(slot !== null) {
                                this.item_drop = null;
                        ...
                        // Player unit is defending unit; player unit defeated enemy unit (via counter attack)
                            ...
                            var slot = this.getFreeSlot(this.targetedUnit);
                            if(slot !== null) {
                                this.item_drop = null;
                                this.item_drop_uses = null;
                ...
                if(this.battleState === 'shopping' || this.battleState === 'buying' || this.battleState === 'selling') {
                    ...
                    if(this.battleState === 'shopping') {
                        if(typeof this.getEntityByName('shop') === 'undefined')
                            this.spawnEntity(EntityOverlay_shop, 0, 0);

                        if(typeof this.getEntityByName('btn_shop_exit') === 'undefined') {
                            this.spawnEntity(EntityButton_shop_exit, this.screen.x + 264, this.screen.y + 132);
                            this.spawnEntity(EntityButton_shop_buy, this.screen.x + 164, this.screen.y + 132);
                            this.spawnEntity(EntityButton_shop_sell, this.screen.x + 364, this.screen.y + 132);
                        }
                    }

                    if(this.battleState === 'selling') {
                        if(typeof this.getEntityByName('btn_shop_item_0') !== 'undefined') {
                            for(var i = 0; i < this.itemCatalog.shop1.length; i++)
                    ...
                    if(this.battleState === 'buying') {
                        ...
                        if(typeof this.getEntityByName('btn_shop_item_0') === 'undefined')
                            for(var i = 0; i < this.itemCatalog.shop1.length; i++)
                                this.spawnEntity(EntityButton_shop_item, this.screen.x + 160, this.screen.y + (i + 1) * 32 + 200, {index: i});
            ...
            if(ig.input.state('rightClick') && !this.menuVisible && this.hoveredUnit == null && this.units[this.activeUnit].unitType === 'player' && this.units[this.activeUnit].vel.x === 0 && this.units[this.activeUnit].vel.y === 0) {
                ...
                this.spawnEntity(EntityButton_suspend, ig.input.mouse.x + this.screen.x, ig.input.mouse.y + this.screen.y + 32);
            }
            ...
            if(!this.scriptCatalog.map0.objective.triggered) {
                // Check end battle map condition
                //this.scriptCatalog.map0.objective.condition();
                this.scriptCatalog.map0.objective.condition(this.turn_counter, 3);
            // End battle
            } else {
        ...
        /**
         *  undefined recomputeStats(object entity)
         *  -----
        ...
        recomputeStats: function(entity) {
            ...
            if(entity.item[0] !== null && entity.item[0].type === 'equip') {
                ...
                var equippedItem = {atk: 0, hit: 0, crit: 0, wt: 0, range: 0, area: 0};
            ...
            if(this.item_drop !== null) {
                ...
                if(this.units[this.activeUnit].unitType === 'player') {
                    for(var i = 0; i < ig.game.units[ig.game.activeUnit].item.length; i++)
                        ig.game.spawnEntity(EntityButton_inventory_item, ig.game.screen.x + 32, ig.game.screen.y + (i + 1) * 32, {index: i});
                } else if(this.units[this.activeUnit].unitType === 'enemy') {
                    for(var i = 0; i < ig.game.targetedUnit.item.length; i++)
                        ig.game.spawnEntity(EntityButton_inventory_item, ig.game.screen.x + 32, ig.game.screen.y + (i + 1) * 32, {index: i});
                }

                ig.game.spawnEntity(EntityButton_dropped_item, ig.game.screen.x + 32, ig.game.screen.y + 296);
            }
        },
        ...
        draw: function() {
            ...
            } else if(this.item_drop !== null && this.units[this.activeUnit] !== 'player' && this.targetedUnit.selectedWeapon !== null && this.targetedUnit.isEquipping) {
                this.weaponModal.draw(390, 350);
                var t = this.targetedUnit;
                t.statMugshot.draw(410, 150);

                var atkChange = t.stat.str + t.selectedWeapon.atk;
                var hitChange = t.stat.hit + t.selectedWeapon.hit;
                var critChange = t.stat.crit + t.selectedWeapon.crit;
                var evadeChange = t.mod.evade;

                this.font20_black.draw('Affinity: ' + t.selectedWeapon.affinity, 510, 370, ig.Font.ALIGN.CENTER);

                // Draw stats for new weapon
                this.font20_black.draw('Atk:', 410, 403);
                if(atkChange > t.derived_stats.atk)
                    this.font16_green.draw(t.derived_stats.atk + '->' + atkChange, 450, 404);
                else if(atkChange < t.derived_stats.atk)
                    this.font16_red.draw(t.derived_stats.atk + '->' + atkChange, 450, 404);
                else
                    this.font16_default.draw(t.derived_stats.atk, 450, 404);

                this.font20_black.draw('Hit:', 410, 437);
                if(hitChange > t.derived_stats.hit_rate)
                    this.font16_green.draw(t.derived_stats.hit_rate + '->' + hitChange, 450, 438);
                else if(hitChange < t.derived_stats.hit_rate)
                    this.font16_red.draw(t.derived_stats.hit_rate + '->' + hitChange, 450, 438);
                else
                    this.font16_default.draw(t.derived_stats.hit_rate, 450, 438);

                this.font20_black.draw('Crit:', 530, 403);
                if(critChange > t.derived_stats.crit_rate)
                    this.font16_green.draw(t.derived_stats.crit_rate + '->' + critChange, 575, 404);
                else if(critChange < t.derived_stats.crit_rate)
                    this.font16_red.draw(t.derived_stats.crit_rate + '->' + critChange, 575, 404);
                else
                    this.font16_default.draw(t.derived_stats.crit_rate, 575, 404);
                ...
                if(this.hoveredUnit !== null && this.attackTargets.length > 0) {
                    for(var i = 0; i < this.attackTargets.length; i++) {
                        if(this.hoveredUnit === this.attackTargets[i]) {
                            ...
                            this.font16_default.draw(this.computeBattleNormalHitChance(this.units[this.activeUnit], this.hoveredUnit) + this.weaponTriangle(this.units[this.activeUnit], this.hoveredUnit),
                                                                                                                                       600, 118                      );
                            this.font16_default.draw(this.computeBattleCritChance(this.units[this.activeUnit], this.hoveredUnit),      600, 149                      );
                            ...
                            this.font16_default.draw(this.computeBattleNormalHitChance(this.hoveredUnit, this.units[this.activeUnit]) + this.weaponTriangle(this.hoveredUnit, this.units[this.activeUnit]),
                                                                                                                                       549, 118, ig.Font.ALIGN.RIGHT );
                            this.font16_default.draw(this.computeBattleCritChance(this.hoveredUnit, this.units[this.activeUnit]),      549, 149, ig.Font.ALIGN.RIGHT );
            ...
            if((this.battleState === 'battle_start' || this.battleState === 'battle_transitionOut') && this.units[this.activeUnit] !== this.targetedUnit) {
                ...
                this.font20_default.draw(this.computeBattleNormalHitChance(a, t) + this.weaponTriangle(a, t),
                ...
                this.font20_default.draw(this.computeBattleNormalHitChance(t, a) + this.weaponTriangle(t, a),
    ...
    ig.GameSettings = ig.BaseGame.extend({
        ...
        init: function(x, y, settings) {
            ...
            this.spawnEntity(EntityButton_settings_vol_down, 360, 112.5);
            this.spawnEntity(EntityButton_settings_vol_up, 480, 112.5);
            this.spawnEntity(EntityButton_settings_return, ig.system.width / 2 - 100, 400);
        },
        ...
        draw: function() {
            ...
            this.font20_default.draw('Game Settings', ig.system.width / 2, 40, ig.Font.ALIGN.CENTER);
            this.font20_default.draw('Volume:  ' + Math.round(ig.music.volume * 100), 60, 120);
    ...
    *   ig.CutScenes
    *   ----
    *       - Displays the cutscenes within the game.
    *       - Play each cutscene before each level begins.
    */
    ig.CutScenes = ig.BaseGame.extend({
        dialogBox : new ig.Image('media/gui/text_box.png'),
        townBackground: new ig.Image('media/backgrounds/town.png'),

        // Set up dialog catalog
        dialogCatalog: new Dialog_Catalog(),
        dialog_index: 0,
        ...
        update: function() {
            ...
            if(ig.input.pressed('leftClick')) {
                if(this.dialog_index < this.dialogCatalog.map1.dialog.length - 1)
                    this.dialog_index++;
                else
        ...
        draw: function() {
            ...
            // Draw the background
            this.dialogCatalog.map1.background.draw(0, 0);

            // Draw the dialog box
            this.dialogBox.draw(0, 360);

            // Draw the speakers
            this.dialogCatalog.map1.speaker1.draw(400, 160);
            this.dialogCatalog.map1.speaker2.draw(40, 160);

            if(this.dialog_index < this.dialogCatalog.map1.dialog.length)
                this.font20_default.draw(this.dialogCatalog.map1.dialog[this.dialog_index], 0, 360);
        }
    });
    ...
    ig.StartScreen = ig.BaseGame.extend({
        ...
        init: function(x, y, settings) {
            ...
            this.spawnEntity(EntityOverlay_startscreen, 0, 0);
            ...
            if(this.storage.isSet('units') && this.storage.isSet('activeUnit'))
                this.spawnEntity(EntityButton_startscreen_continue, 480, 280 + i++ * 40);

            this.spawnEntity(EntityButton_startscreen_newgame, 480, 280 + i++ * 40);
            this.spawnEntity(EntityButton_startscreen_settings, 480, 280 + i++ * 40);
            this.spawnEntity(EntityButton_startscreen_credits, 480, 280 + i++ * 40); // TODO
    ...
    ig.GameOver = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            this.font20_default.draw('Game Over', ig.system.width / 2, 160, ig.Font.ALIGN.CENTER);
            this.font16_default.draw(ig.global.main_player_name + ' has been defeated.', ig.system.width / 2, 220, ig.Font.ALIGN.CENTER);
            this.font16_default.draw('Left click to start', ig.system.width / 2, 400, ig.Font.ALIGN.CENTER);
        } // End draw method
    }); // End ig.GameOver

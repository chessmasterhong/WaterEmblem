...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.input.bind(ig.KEY.H,           'hpmodal'); // Show the hp bar modal for active unit.
            ig.input.bind(ig.KEY.T,           'terrainModal'); // Show the Terrain modal on key press.
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if((ig.input.pressed('escape') && this.menuVisible) || this.units[this.activeUnit].unitType !== 'player') {
                this.clickedUnit = null;
        ...
        draw: function() {
            ...
            if(ig.input.state('menu') && this.units[this.activeUnit].unitType === 'player') {
                ...
                ig.game.units[ig.game.activeUnit].statMugshot.draw(30, 40);
            ...
            // Filler code to draw small character modal, showing small sprite, HP and Level.
            // Right now it draws on key press 'H', but we'd like to spawn this whenever the
            // cursor is hovering on top of an entity. Currently, we'll only focus on player
            // modals.
            //if(ig.input.state('hpmodal') && this.units[this.activeUnit].unitType === 'player'){
            if(this.units[this.activeUnit].unitType === 'player' && this.clickedUnit === null) {
                //var a = this.units[this.activeUnit];
                // Draw small status screen modal
                this.hpModal.draw(0, 400);
                // Draw small character modal sprite
                ig.game.units[ig.game.activeUnit].modal.draw(5, 420);
            }

            // Draw the terrain modal on key press. For now, we use a key press for testing.
            // In the future, we would spawn this whenever a unit is not selected by the
            // cursor entity. "Free moving state"
            //if(ig.input.state('terrainModal') && this.units[this.activeUnit].unitType === 'player'){

...
.defines(function() {
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            this.units = enemy_spawner.spawnEnemies(enemy_spawner.generateEnemies(2, 4));
            ...
            for(var p = 0; p < this.partyMembers; p++) {
                party[p] = this.spawnEntity(
                    ...
                    this.mapsize.y * 0.25 * (p+1) + t - (this.mapsize.y * 0.25 * (p+1) + t) % ig.global.tilesize - ig.global.tilesize,
                    {name: 'player_' + p}
                );
            }

    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            if(this.units[this.activeUnit].selectedWeapon !== null && this.units[this.activeUnit].isEquipping) {
                ...
                this.font20_black.draw('Affinity: ' + a.selectedWeapon.affinity, 520, 370, ig.Font.ALIGN.CENTER);

                // Draw stats for new weapon
                this.font20_black.draw('Atk:', 420, 403);
                if(atkChange > a.derived_stats.atk)
                    this.font16_green.draw(a.derived_stats.atk + '->' + atkChange, 460, 404);
                else if(atkChange < a.derived_stats.atk)
                    this.font16_red.draw(a.derived_stats.atk + '->' + atkChange, 460, 404);
                else
                    this.font16_default.draw(a.derived_stats.atk, 460, 404);

                this.font20_black.draw('Hit:', 420, 437);
                if(hitChange > a.derived_stats.hit_rate)
                    this.font16_green.draw(a.derived_stats.hit_rate + '->' + hitChange, 460, 438);
                else if(hitChange < a.derived_stats.hit_rate)
                    this.font16_red.draw(a.derived_stats.hit_rate + '->' + hitChange, 460, 438);
                else
                    this.font16_default.draw(a.derived_stats.hit_rate, 460, 438);

                this.font20_black.draw('Crit:', 540, 403);
                if(critChange > a.derived_stats.crit_rate)
                    this.font16_green.draw(a.derived_stats.crit_rate + '->' + critChange, 580, 404);
                else if(critChange < a.derived_stats.crit_rate)
                    this.font16_red.draw(a.derived_stats.crit_rate + '->' + critChange, 580, 404);
                else
                    this.font16_default.draw(a.derived_stats.crit_rate, 580, 404);

                this.font20_black.draw('Avo:', 540, 437);
                this.font16_default.draw(evadeChange, 580, 438);
            }

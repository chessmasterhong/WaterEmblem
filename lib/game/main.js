...
.defines(function() {
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        draw: function() {
            ...
            // Menu to prematurely end player's phase (end all player's active unit turn)
            //if(ig.input.state('escape') && this.units[this.activeUnit].unitType === 'player') {
            if(ig.input.state('escape') && this.units[this.activeUnit].unitType === 'player' && !this.showingMenu) {
            ...
            if(ig.input.state('items') && this.units[this.activeUnit].unitType === 'player' && !this.showingMenu) {
                ...
                ig.game.spawnEntity(Button, ig.game.screen.x + 75, ig.game.screen.y + 65, {
                    ...
                    pressedDown: function() {
                        console.log("Time to spawn the items");
                        ig.game.showingMenu = false;
                        // Log the amount of items the Active Player has
                        console.log(ig.game.units[ig.game.activeUnit].items.length);


                        // First Item
                        //for(var i = 0; i < ig.game.units[this.activeUnit].items.length; i++){
                        //    console.log(ig.game.units[this.activeUnit].items.length);
                        //}
                        for(var i = 1; i < ig.game.units[ig.game.activeUnit].items.length + 1; i++){

                                ig.game.spawnEntity( Button, ig.game.screen.x + 100, ig.game.screen.y + i*65, {
                                font: new ig.Font('media/arial.font_14.png'),
                                text: ['Item 1'],
                                textPos: { x: 37, y: 8 },
                                textAlign: ig.Font.ALIGN.CENTER,

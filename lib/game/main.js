...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.input.bind(ig.KEY.C,           'onfocus'); //C: when c press camera follows on pointer , if press again camera follows active player
    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(ig.input.pressed('escape') || this.units[this.activeUnit].unitType !== 'player') {
                if(this.clickedUnit !== null && this.clickedUnit.unitType === 'player' && this.clickedUnit.vel.x === 0 && this.clickedUnit.vel.y === 0) {
                    if(this.clickedUnit.pos !== this.clickedUnit.init_pos) {
                        ...
                        ig.game.clickedUnit = this.units[this.activeUnit];
            ...
            var pointer = this.getEntityByName('pointer');
            
            if(ig.input.pressed('onfocus') && this.onUnit==false)
            {
                this.onUnit= true;
            }
            else if(ig.input.pressed('onfocus') && this.onUnit==true)
                {
                    this.onUnit=false;
                    console.log("focus on unit player");
                }
           
           if(this.onUnit == true)
            {
                console.log("focus on pointer");
                this.camera.follow(pointer);
            }

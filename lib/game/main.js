    ...
    **/
    BattleMode = BaseGame.extend({
        ...
        init: function() {
            ...
            num_of_enemies_range = {min: 2, max: 4};
            ...
            for(var e = 0; e < num_of_enemies; e++) {
                var spawned_enemy = this.spawnEntity(enemies[e].entity_name, enemies[e].pos.x, enemies[e].pos.y, {name: 'enemy_' + e});
        ...
        update: function() {
            ...
            var enemy_count = num_of_enemies;
        ...
        ////////////////////////////////////////////////////////////////////////
        // array generateEnemies(num_of_enemies, pos_of_enemies_range_min_x, pos_of_enemies_range_max_x, pos_of_enemies_range_min_y, pos_of_enemies_range_max_y)
        //     Generates enemies based on encounter zone.
        //
        // Precondition:
        //     num_of_enemies: The number of enemies to generate. Must be an
        //         integer greater than 0.
        //     pos_of_enemies_range_min_x: The minimum x-coordinate of the
        //         bounding box to generate enemy's position. Must be an integer
        //         and pos_of_enemies_range_min_x <= pos_of_enemies_range_max_x.
        //     pos_of_enemies_range_max_x: The maximum x-coordinate of the
        //         bounding box to generate enemy's position. Must be an integer
        //         and pos_of_enemies_range_min_x <= pos_of_enemies_range_max_x.
        //     pos_of_enemies_range_min_y: The minimum y-coordinate of the
        //         bounding box to generate enemy's location. Must be an integer
        //         and pos_of_enemies_range_min_y <= pos_of_enemies_range_max_y.
        //     pos_of_enemies_range_max_y: The maximum y-coordinate of the
        //         bounding box to generate enemy's position. Must be an integer
        //         and pos_of_enemies_range_min_y <= pos_of_enemies_range_max_y.
        //
        // Postcondition:
        ...
        //     enemy's battle data based on the encounter zone. Enemy's data can
        //     be obtained by getting the object's property from the array
        //     element.
        //     Example:
        //         // Generate 2 random enemies from encounter zone 4 and randomly
        //         // position them between 0 <= x <= 100 and 50 <= y <= 150.
        //         ig.global.encounter_zone = 4;
        //         var enemies = generateEnemies(2, 0, 100, 50, 150);
        //         // Get second enemy's x-coordinates.
        //         enemies[1].pos.x;
        ...
        generateEnemies: function(num_of_enemies, pos_of_enemies_range_min_x, pos_of_enemies_range_max_x, pos_of_enemies_range_min_y, pos_of_enemies_range_max_y) {
            ...
            var range_min = {x: Math.abs(pos_of_enemies_range_min_x), y: Math.abs(pos_of_enemies_range_min_y)};
            var range_max = {x: Math.abs(pos_of_enemies_range_max_x), y: Math.abs(pos_of_enemies_range_max_y)};

            // Create enemies
            var enemies = [];
            if((typeof num_of_enemies === 'number' && (num_of_enemies % 1) === 0) &&
               (typeof range_min.x === 'number' && (range_min.x % 1) === 0) &&
               (typeof range_max.x === 'number' && (range_max.x % 1) === 0) &&
               (typeof range_min.y === 'number' && (range_min.y % 1) === 0) &&
               (typeof range_max.y === 'number' && (range_max.y % 1) === 0) &&
               (num_of_enemies > 0 && range_min.x <= range_max.x && range_min.y <= range_max.y)) {
                for(var e = 0; e < num_of_enemies; e++) {
                    // Randomize spawn position
                    p = {x: range_min.x + Math.floor(Math.random() * (range_max.x - range_min.x)),
                    ...
                    enemies[e] = {
                        entity_name: enemy_groups[Math.floor(Math.random() * enemy_groups_size)], // Select a random enemy from the encounter zone enemies; Used to reference entity to spawn
                        pos: {x: p.x + t - (p.x + t) % tilesize, y: p.y + t - (p.y + t) % tilesize}, // Round spawn position to nearest multiple of tile size; Used to set entity's position
                    };
                }
            }

    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(this.units[this.activeUnit] !== this.targetedUnit) {
                if(this.battleAnimStage === 'attack') {
                ...
                } else if(this.battleAnimStage === 'assist') {
                ...
                } else if(this.battleAnimStage === 'trade') {
                    ...
                    if(this.units[this.activeUnit].unitType === 'player' && this.targetedUnit.unitType === 'player') {
                        ...
                        for(var t = 0; t < this.assistTargets.length; t++) {
                            if(this.targetedUnit === this.assistTargets[t]) {
                                ...
                                // TODO: Add trade functionality here
                                break;
                            }
                        }
                    }
                    ...
                    if(this.attackQueueIndex === 0 && this.atkAnimTimer.delta() > 1) {
                        ...
                        // Active unit is playing their attack animation and is on animation frame just before striking
                            ...
                            if(this.units[this.activeUnit].derived_stats.crit_rate - this.targetedUnit.stat.luk > (Math.random()*100)) {
                                console.log("Critical hit");
                                // TODO: Active unit's critical attack animation goes here
                                this.screenshakeIntensity = 10;
                                this.targetedUnit.receiveDamage((this.units[this.activeUnit].derived_stats.atk - this.targetedUnit.stat.def) * 3, this.units[this.activeUnit]); // Inflict critical damage
                            // No critical hit triggered, proceed to normal attack; compute if active unit lands a normal hit on targeted unit
                            } else if(this.units[this.activeUnit].derived_stats.hit_rate - this.targetedUnit.derived_stats.evade > (Math.random()*100)) {
                                // TODO: Active unit's normal attack animation goes here
                                this.screenshakeIntensity = 5;
                                this.targetedUnit.receiveDamage(this.units[this.activeUnit].derived_stats.atk - this.targetedUnit.stat.def, this.units[this.activeUnit]); // Inflict normal damage
                            } else {
                    ...
                    // Counterattack -----------------------------------------------
                        ...
                        // Target unit is playing their attack animation and is on animation frame just before striking
                            ...
                            if(this.targetedUnit.derived_stats.crit_rate - this.units[this.activeUnit].stat.luk > (Math.random()*100)) {
                                console.log("Critical Hit");
                                // TODO: Targeted unit's critical attack animation goes here
                                this.screenshakeIntensity = 10;
                                this.units[this.activeUnit].receiveDamage((this.targetedUnit.derived_stats.atk - this.units[this.activeUnit].stat.def) * 3, this.targetedUnit); // Inflict critical damage
                            // No critical hit triggered, proceed to normal attack; compute if targeted unit lands a normal hit on active unit
                            } else if(this.targetedUnit.derived_stats.hit_rate - this.units[this.activeUnit].derived_stats.evade > (Math.random()*100)) {
                                // TODO: Targeted unit's normal attack animation goes here
                                this.screenshakeIntensity = 5;
                                this.units[this.activeUnit].receiveDamage(this.targetedUnit.derived_stats.atk - this.units[this.activeUnit].stat.def, this.targetedUnit); // Inflict normal damage
                            } else {
                                console.log('Attack missed');
                            ...
                            // Faster unit is playing their attack animation and is on animation frame just befor striking
                                ...
                                if(this.units[this.activeUnit].stat.spd > this.targetedUnit.stat.spd) {
                                    ...
                                    if(this.units[this.activeUnit].derived_stats.crit_rate - this.targetedUnit.stat.luk > (Math.random()*100)) {
                                        ...
                                        this.targetedUnit.receiveDamage((this.units[this.activeUnit].derived_stats.atk - this.targetedUnit.stat.def) * 3, this.units[this.activeUnit]); // Inflict critical damage
                                    // No critical hit triggered, proceed to normal attack; compute if active unit lands a normal hit on targeted unit
                                    } else if(this.units[this.activeUnit].derived_stats.hit_rate - this.targetedUnit.derived_stats.evade > Math.random()) {
                                        // TODO: Active unit's normal attack animation goes here
                                        this.screenshakeIntensity = 5;
                                        this.targetedUnit.receiveDamage(this.units[this.activeUnit].derived_stats.atk - this.targetedUnit.stat.def, this.units[this.activeUnit]); // Inflict normal damage
                                    } else {
                                ...
                                // Targeted unit is faster unit; targeted unit attacks second time
                                    ...
                                    if(this.targetedUnit.derived_stats.crit_rate - this.units[this.activeUnit].stat.luk > (Math.random()*100)) {
                                        ...
                                        this.units[this.activeUnit].receiveDamage((this.targetedUnit.derived_stats.atk - this.units[this.activeUnit].stat.def) * 3, this.targetedUnit); // Inflict critical damage
                                    // No critical hit triggered, proceed to normal attack; compute if targeted unit lands a normal hit on active unit
                                    } else if(this.targetedUnit.derived_stats.hit_rate - this.units[this.activeUnit].derived_stats.evade > Math.random()) {
                                        // TODO: Targeted unit's normal attack animation goes here
                                        this.screenshakeIntensity = 5;
                                        this.units[this.activeUnit].receiveDamage(this.targetedUnit.derived_stats.atk - this.units[this.activeUnit].stat.def, this.targetedUnit); // Inflict normal damage
                                    } else {
        ...
        recomputeStats: function(entity) {
            ...
            entity.derived_stats.atk = entity.stat.str + equippedItem.atk;
            // Compute hit_rate = base skl / 2 + base luk / 2 + weapon hit rate
            entity.derived_stats.hit_rate = entity.stat.skl / 2 + entity.stat.luk / 2 + equippedItem.hit;
            // Compute crit rate = base skl / 2 + weapon crit rate
            entity.derived_stats.crit_rate = entity.stat.skl / 2 + equippedItem.crit;
            ...
            if(entity.stat.wt > equippedItem.wt) {
                ...
                entity.derived_stats.attack_speed = entity.stat.spd;
            } else {
                // Attack speed = base spd - (weapon weight - player base wt stat)
                entity.derived_stats.attack_speed = entity.stat.spd - (equippedItem.wt - entity.stat.wt);
            }
            // Compute evade = attack_speed * 2 + base spd
            entity.derived_stats.evade = entity.derived_stats.attack_speed * 2 + entity.stat.spd;
        ...
        // HIT RATE = Skill x 2 + Luck / 2
        // We already computed and stored the hit rate bonus from
        // the weapon inside of the unit's derived stats.
        // Here, we just compute the base hit rate from stats.
        computeHitRate: function(skill, luck) {
            ig.game.units[ig.game.activeUnit].stat.hit += skill / 2;
            ig.game.units[ig.game.activeUnit].stat.hit += luck / 2;
            return ig.game.units[ig.game.activeUnit].stat.hit;
        },
        ...
        computeEvade: function(attack_speed, luck, terrain_bonus) {
            ig.game.units[ig.game.activeUnit].mod.evade += attack_speed;
            ig.game.units[ig.game.activeUnit].mod.evade += luck;
            ig.game.units[ig.game.activeUnit].mod.evade += terrain_bonus;
            return ig.game.units[ig.game.activeUnit].mod.evade;
        },
        ...
        computeCriticalHitRate: function(skill) {
            /*
                Paramters:
            ...
            */
            var critical = 0;
            return critical = skill / 2;
        },

        // Method for trading items between two different players.
        tradeItems: function(array_a, array_b, index_a, index_b) {
            var selectedItemToTrade = array_a[index_a]; // Store the item unit A wants to trade unit B
            var selectedItemForTrade = array_b[index_b]; // Store the item unit A wants from unit B
            array_b[index_b] = selectedItemToTrade; // Move selected item from unit A index to unit B index
            array_a[index_a] = selectedItemForTrade; // Move the item from unit B's inventory into unit A's inventory
        ...
        draw: function() {
            if(this.screenshakeIntensity === 0)
                ...
                ig.system.context.save();
                    ...
                    ig.system.context.translate(
                        this.screenshakeIntensity * this.sgn(Math.random() - 0.5),
                        this.screenshakeIntensity * this.sgn(Math.random() - 0.5));
                    this.parent();
            ...
            if(ig.input.state('menu') && this.units[this.activeUnit].unitType === 'player' && this.battleAnimStage !== 'battle_start') {
            ...
            if(this.units[this.activeUnit].selectedWeapon !== null && this.units[this.activeUnit].isEquipping) {
                ...
                this.font.draw(a.derived_stats.atk + "->" + atkChange, 440, 395);
                this.font.draw(a.derived_stats.hit_rate + "->" + hitChange, 435, 425);
                this.font.draw(a.derived_stats.crit_rate + "->" + critChange, 530, 395);
                this.font.draw(evadeChange, 540, 425);
            ...
            if(this.units[this.activeUnit].unitType === 'player' && this.battleAnimStage !== 'battle_start' && this.hoveredUnit !== null) {
            ...
            if(this.units[this.activeUnit].unitType === 'player' && this.battleAnimStage !== 'battle_start' && this.clickedUnit === null && this.hoveredTerrain !== null) {
                ...
                if(this.hoveredUnit !== null && this.attackTargets.length > 0) {
                    for(var i = 0; i < this.attackTargets.length; i++) {
                        if(this.hoveredUnit === this.attackTargets[i]) {
                            ...
                            this.font.draw(this.units[this.activeUnit].name, 565, 10);
                            this.font.draw(this.units[this.activeUnit].health, 610, 45);
                            this.font.draw(this.hoveredUnit.health, 530, 45);
                            this.font.draw(this.units[this.activeUnit].derived_stats.atk - this.hoveredUnit.stat.def, 610, 80);
                            this.font.draw(this.hoveredUnit.derived_stats.atk - this.units[this.activeUnit].stat.def, 540, 80);
                            this.font.draw(this.units[this.activeUnit].derived_stats.hit_rate - this.hoveredUnit.derived_stats.evade, 610, 115);
                            this.font.draw(this.hoveredUnit.derived_stats.hit_rate - this.units[this.activeUnit].derived_stats.evade, 530, 115);
                            this.font.draw(this.units[this.activeUnit].derived_stats.crit_rate - this.hoveredUnit.stat.luk, 530, 150);
                            this.font.draw(this.hoveredUnit.derived_stats.crit_rate - this.units[this.activeUnit].stat.luk, 610, 150);

                            // Compute all stats, put them in
                            // this.hoveredUnit.stat.atk -> Reference to other entity
                            }
                        }
                    }
                }
            ...
            if((this.battleAnimStage === 'battle_prepare' || this.battleAnimStage === 'battle_start') && this.units[this.activeUnit] !== this.targetedUnit) {
                ...
                if(a.unitType === 'player' && t.unitType === 'enemy') {
                    var a_overlayDetailPos = [
                        ...
                        {x: ig.system.width / 2 + 25, y: 385},  // Weapon image
                ...
                this.font.draw(a.derived_stats.hit_rate - t.derived_stats.evade, a_overlayDetailPos[1].x, a_overlayDetailPos[1].y, a_overlayDetailPos[1].align);
                this.font.draw(a.derived_stats.atk - t.stat.def,                 a_overlayDetailPos[2].x, a_overlayDetailPos[2].y, a_overlayDetailPos[2].align);
                this.font.draw(a.derived_stats.crit_rate - t.stat.luk,           a_overlayDetailPos[3].x, a_overlayDetailPos[3].y, a_overlayDetailPos[3].align);
                ...
                this.font.draw(t.derived_stats.hit_rate - a.derived_stats.evade, t_overlayDetailPos[1].x, t_overlayDetailPos[1].y, t_overlayDetailPos[1].align);
                this.font.draw(t.derived_stats.atk - a.stat.def,                 t_overlayDetailPos[2].x, t_overlayDetailPos[2].y, t_overlayDetailPos[2].align);
                this.font.draw(t.derived_stats.crit_rate - a.stat.luk,           t_overlayDetailPos[3].x, t_overlayDetailPos[3].y, t_overlayDetailPos[3].align);

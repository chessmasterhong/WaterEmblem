    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        swapItems: function(array, a, b) {
         ...
         equipWeapon: function(atk, wt, crit, hit){
            /*
                Parameters:
        ...
        applyWeapon: function(base_atk, base_wt, base_crit, base_hit){
            /*
                Paramters:
                    - base_atk: input atk fed in from active unit base atk
                    - base_wt: input wt fed in from active unit base def
                    - base_crit: input crit fed in from active unit base crit
                    - base_hit: input hit fed in from active unit base hit
            */
            ig.game.units[ig.game.activeUnit].stat.atk = ig.game.units[ig.game.activeUnit].mod.atk + base_atk;
            ig.game.units[ig.game.activeUnit].stat.hit = ig.game.units[ig.game.activeUnit].mod.hit + base_hit;
            ig.game.units[ig.game.activeUnit].stat.wt  = ig.game.units[ig.game.activeUnit].mod.wt + base_wt;
            ig.game.units[ig.game.activeUnit].stat.crit = ig.game.units[ig.game.activeUnit].mod.crit + base_crit;
            return ig.game.units[ig.game.activeUnit].stat;
        ...
        computeAttackSpeed: function(speed, weight, weapon_weight){
            ...
            if(weight < weapon_weight){
                ...
                this.attackSpeed += (weapon_weight - weight);
        ...
        draw: function() {
            ...
            if(this.units[this.activeUnit].isEquipping){
                ...
                var a = this.units[this.activeUnit];
                var atkChange = a.stat.atk + a.selectedWeapon.atk;
                var hitChange = a.stat.hit + a.selectedWeapon.hit;
                var critChange = a.stat.crit + a.selectedWeapon.crit;
                // Draw stats for new weapon
                this.font.draw(a.stat.atk + "->" + atkChange, 445, 395);
                this.font.draw(a.stat.hit + "->" + hitChange, 440, 425);
                this.font.draw(a.stat.crit + "->" + critChange, 530, 395);

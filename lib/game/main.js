    ...
    **/
    BattleMode = BaseGame.extend({
        ...
        init: function() {
            ...
            var mapsize = {x: this.collisionMap.width * tilesize, y: this.collisionMap.height * tilesize};
            var pos_of_enemies_range = {min: {x: tilesize, y: tilesize},
                                        max: {x: mapsize.x/2, y: mapsize.y}}
            ...
            for(var e = 0; e < num_of_enemies; e++) {
                enemy[e] = this.spawnEntity(enemies[e].name, enemies[e].pos.x, enemies[e].pos.y);
                console.log(enemies[e].name + ' spawned at (' + enemy[e].pos.x + ', ' + enemy[e].pos.y + ')')
            }
        },
        ...
        generateEnemies: function(num_of_enemies, pos_of_enemies_range_min_x, pos_of_enemies_range_max_x, pos_of_enemies_range_min_y, pos_of_enemies_range_max_y) {
            ...
            for(var e = 0; e < num_of_enemies; e++) {
                enemies[e] = {
                    ...
                    pos: {x: pos_of_enemies_range_min_x + pos_of_enemies_range_min_x * Math.floor(((Math.random() * (pos_of_enemies_range_max_x - pos_of_enemies_range_min_x)) / tilesize)),
                          y: pos_of_enemies_range_min_y + pos_of_enemies_range_min_y * Math.floor(((Math.random() * (pos_of_enemies_range_max_y - pos_of_enemies_range_min_y)) / tilesize))},
                    health: 10,
                    ap: 4,
                    type: 'Swordsman'
                };

...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            //ig.input.bind(ig.KEY.ENTER,       'select'); // Enter: select option, interact with object/character, continue dialogue
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        init: function() {
            ...
            party[0] = this.spawnEntity(
                EntityHero,
                this.mapsize.x * 0.65 + t - (this.mapsize.x * 0.65 + t) % ig.global.tilesize + ig.global.tilesize,
                this.mapsize.y * 0.5  + t - (this.mapsize.y * 0.5  + t) % ig.global.tilesize - ig.global.tilesize,
                {name: 'player_main'}
            );
            ...
            for(var p = 1; p <= this.partyMembers; p++) {
                party[p] = this.spawnEntity(
                    EntityAlly,
                    this.mapsize.x * 0.7 + t - (this.mapsize.x * 0.7 + t) % ig.global.tilesize + ig.global.tilesize,
                    this.mapsize.y / (this.partyMembers + 1) * p + t - (this.mapsize.y / (this.partyMembers + 1) * p + t) % ig.global.tilesize - ig.global.tilesize,
                    {name: 'player_' + p}
                );
            }
        ...
        update: function() {
            ...
            if(this.units[this.activeUnit].turnUsed) {
                ...
                if(this.units[this.activeUnit].unitType === 'party')

...
.defines(function() {
    ...
    ig.BaseGame = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.music.volume = 0.3;
            this.auxMusic.volume = 0.3;
            this.sfx.volume = 0.9;

            ig.music.loop = true;
            this.auxMusic.loop = true;
    ...
    ig.BattleMode = ig.BaseGame.extend({
            ...
            if(ig.global.gameState !== 'continue') {
                ...
                for(var u = 0; u < units_tmp.length; u++) {
                    this.units[u] = this.spawnEntity(units_tmp[u].entityClassName[0], units_tmp[u].pos.x, units_tmp[u].pos.y, {
                        ...
                        exp_curr:               units_tmp[u].exp_curr || 0,
                        hasEquippedItem:        units_tmp[u].hasEquippedItem,
                        health:                 units_tmp[u].health,
                        health_max:             units_tmp[u].health_max,
                        immovable:              units_tmp[u].immovable,
                        item:                   units_tmp[u].item,
                        item_uses:              units_tmp[u].item_uses,
                        level:                  units_tmp[u].level,
                        levelUpBonuses:         units_tmp[u].levelUpBonuses,
                        mod:                    units_tmp[u].mod,
                        mov_curr:               units_tmp[u].mov_curr || 0,
                        name:                   units_tmp[u].name,
                        //resist:                 units_tmp[u].resist || {},
                        stat:                   units_tmp[u].stat,
                        //status:                 units_tmp[u].status,
                        terrainMod:             units_tmp[u].terrainMod,
                        waitUntilTurn:          units_tmp[u].waitUntilTurn || 0,
                    });
                }
        ...
        fadeInFromBattleAnim: function() {
            ...
            this.screenFadeInFromBattleAnim = new ig.ScreenFader({
                ...
                callback: function() {
                    ...
                    if(this.battleState === 'player_phase') {
                        ig.music.play(this.levels[ig.global.currentLevel].music);
                    } else {
                        ig.music.play('augustine_army');
                    }
        ...
        computeExperience: function(attacker, target, killed) {
            var exp = killed ? 75 : Math.floor((32 + attacker.level + target.level ) / 4);

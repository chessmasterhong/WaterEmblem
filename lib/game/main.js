    ...
    */
    ...
    ig.BattleMode = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(!this.levels[this.director.currentLevel].objective_triggered) {
                // Check end battle map condition
                if(this.levels[this.director.currentLevel].objective()) {
                    this.levels[this.director.currentLevel].objective_triggered = true;

                    console.log('Leaving battle mode.');
                    ig.game.fadeIn(function() {
        ...
        recomputeStats: function(entity) {
            ...
            var equippedItem;
            if(entity.item[0] !== null && entity.item[0].type === 'equip' && this.isValidWeapon(entity.item[0].affinity, entity)) {
                equippedItem = {
    ...
    ig.CutScene = ig.BaseGame.extend({
        ...
        update: function() {
            ...
            if(ig.input.pressed('leftClick')) {
                if(this.dialog_index < this.dialogCatalog[ig.global.currentLevel].dialog.length - 1)
                    this.dialog_index++;
                else
                    ig.system.setGame(ig.BattleMode);
            }
        ...
        draw: function() {
            ...
            // Draw the background
            this.dialogCatalog[ig.global.currentLevel].background.draw(0, 0);

            // Draw the dialog box
            this.dialogBox.draw(0, 360);

            // Draw the speakers
            this.dialogCatalog[ig.global.currentLevel].speaker1.draw(400, 160);
            this.dialogCatalog[ig.global.currentLevel].speaker2.draw(40, 160);

            if(this.dialog_index < this.dialogCatalog[ig.global.currentLevel].dialog.length)
                this.font20.default.draw(this.dialogCatalog[ig.global.currentLevel].dialog[this.dialog_index], 0, 360);
        }
    });

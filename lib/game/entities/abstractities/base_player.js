...
.defines(function() {
    ...
    ig.global.EntityBase_player = ig.Entity.extend({
        ...
        mod: {
            ...
            action_pts: 0, phy_dam: 0, mag_dam: 0, phy_def: 0, mag_def: 0, acc: 0, eva: 0,
        ...
        init: function(x, y, settings) {
            ...
            this.health_max        = Math.floor(-1 + this.level * 1 + this.stat.def * 1                                         + this.mod.health_max                                                 );
            this.stat.phy_dam      = Math.floor((    this.level * 1 + this.stat.atk * 0.5 + this.stat.atk * 1   * Math.random() + this.mod.phy_dam     ) / (1 + (1 - Number(this.status.isParalyzed))));
            this.stat.mag_dam      = Math.floor((    this.level * 1 + this.stat.mag * 0.5 + this.stat.atk * 1   * Math.random() + this.mod.mag_dam     ) / (1 + (1 - Number(this.status.isDazed)))    );
            this.stat.phy_def      = Math.floor(                      this.stat.def * 0.5                                       + this.mod.phy_def                                                    );
            this.stat.mag_def      = Math.floor(                      this.stat.res * 0.5                                       + this.mod.mag_def                                                    );
            this.stat.acc          = Math.floor((1 - this.level * 1 + this.stat.spd * 1                                         + this.mod.acc         ) / (1 + (1 - Number(this.status.isBlinded)))  );
            this.stat.eva          = Math.floor((1 - this.level * 1 + this.stat.spd * 1                                         + this.mod.eva         ) / (1 + (1 - Number(this.status.isBlinded)))  );
            //this.stat.crit         = Math.floor(                      this.stat.luk * 1                                         + this.mod.crit                                                       );

            // Resistances
            this.resist.poison   = Math.floor(this.stat.def * 0.75                        + this.stat.luk * 0.1  + this.mod.poison  );
            this.resist.blind    = Math.floor(this.stat.mag * 0.75                        + this.stat.luk * 0.1  + this.mod.blind   );
            this.resist.paralyze = Math.floor(this.stat.spd * 0.5  + this.stat.spd * 0.25 + this.stat.luk * 0.1  + this.mod.paralyze);
            this.resist.daze     = Math.floor(this.stat.res * 0.75                        + this.stat.luk * 0.1  + this.mod.daze    );
            this.resist.sleep    = Math.floor(this.stat.mag * 0.25 + this.stat.res * 0.5  + this.stat.luk * 0.1  + this.mod.sleep   );
            this.resist.confuse  = Math.floor(this.stat.mag * 0.25 + this.stat.res * 0.5  + this.stat.luk * 0.1  + this.mod.confuse );

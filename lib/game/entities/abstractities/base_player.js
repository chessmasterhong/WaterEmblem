                        ...
                        // If unit used up all their movement points, end their turn
                            ...
                            if(typeof ig.game.getEntityByName('btn_wait') === 'undefined' && !ig.game.menuVisible) {
                                ...
                                if(this.item[0] !== null) {
                                    if(this.item[0].affinity !== 'staff') {
                                        ...
                                        for(i = 0; i < assist_tile_search.length; i++) {
                                            // Spawn dummy tile entities at nearby tiles and run each of their checkEntities() methods.
                                            ent = ig.game.spawnEntity(EntityTile_dummy, assist_tile_search[i].pos_x + ig.global.tilesize / 2, assist_tile_search[i].pos_y + ig.global.tilesize / 2);
                                ...
                                for(i = 0; i < trade_tile_search.length; i++) {
                                    // Spawn dummy tile entities at nearby tiles and run each of their checkEntities() methods.
                                    ent = ig.game.spawnEntity(EntityTile_dummy, trade_tile_search[i].pos_x + ig.global.tilesize / 2, trade_tile_search[i].pos_y + ig.global.tilesize / 2);
                                ...
                                if(ig.game.item_drop === null) {
                                    ...
                                    if(ig.game.getTerrain(this.pos.x, this.pos.y).type === 'Shop' && ig.game.units[ig.game.activeUnit] instanceof EntityHero)
                                        ig.game.spawnEntity(EntityButton_seize, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); //Seize buttong
                                }
                                // End menus -----------------------------------
                            }
                        }

...
.defines(function() {
    ...
    ig.global.EntityBase_player = ig.Entity.extend({
        ...
        derived_stats: {
            atk: 0, // Computed by adding mods from weapons + base stat STR.
            hit_rate: 0, // Computed by mods from weapons + base stat HIT.
        ...
        // Holds the selected weapon for possible equiping
        selectedWeapon: null,
        // Might be necessary for future durability checks
        equippedWeapon: null,
        // Are we currently inside of the equipping state?
        isEquipping: false,
                                        ...
                                        if(typeof ig.game.getEntityByName('btn_item_0') === 'undefined') {
                                            ...
                                            for(var i = 0; i < ig.game.units[ig.game.activeUnit].item.length; i++) {
                                                ...
                                                ig.game.spawnEntity(Button, ig.game.screen.x + 128, ig.game.screen.y + (i + 1) * 32, {
                                                    ...
                                                    index: i,
                                                    font: new ig.Font('media/arial.font_14.png'),
                                                    text: [ig.game.units[ig.game.activeUnit].item[i].name],
                                                    textPos: {x: 37, y: 8},
                                                    textAlign: ig.Font.ALIGN.CENTER,
                                                    size: {x: 75, y: 23},
                                                    animSheet: new ig.AnimationSheet('media/gui/button.png', 75, 23),
                                                    ...
                                                    pressedDown: function() {
                                                        ...
                                                        if(typeof ig.game.getEntityByName('btn_equip') === 'undefined') {
                                                            ...
                                                            console.log(ig.game.units[ig.game.activeUnit].selectedWeapon.name);
                                                            ...
                                                            ig.game.spawnEntity(Button, ig.game.screen.x + 224, ig.game.screen.y + 32, {
                                                                ...
                                                                pressedDown: function() {
                                                                    ...
                                                                    if(ig.game.units[ig.game.activeUnit].selectedItemIndex === 0) {
                                                                        ...
                                                                        // Clear the modifiers applied through weapons
                                                                        ig.game.units[ig.game.activeUnit].mod = ig.game.clearMod();

                                                                        // Accumulate the mod with the weapons stats through the equipWeapon function
                                                                        var buff = ig.game.equipWeapon(ig.game.units[ig.game.activeUnit].selectedWeapon.atk, 
                                                                            ig.game.units[ig.game.activeUnit].selectedWeapon.hit,
                                                                            ig.game.units[ig.game.activeUnit].selectedWeapon.wt, 
                                                                            ig.game.units[ig.game.activeUnit].selectedWeapon.crit);
                                                                        // Properly resets stats
                                                                        console.log("Hit rate buffered: " + buff.hit);

                                                                        
                                                                        
                                                                        // Accumulate the new stats from the weapon mods + base stats, store them inside of
                                                                        // Derived stats
                                                                        var newStats = ig.game.applyWeapon(ig.game.units[ig.game.activeUnit].stat.str, 
                                                                            ig.game.units[ig.game.activeUnit].stat.wt, ig.game.units[ig.game.activeUnit].stat.hit,
                                                                            ig.game.units[ig.game.activeUnit].stat.crit);

                                                                        // Compute attack speed of active unit
                                                                        ig.game.units[ig.game.activeUnit].derived_stats.attack_speed = ig.game.computeAttackSpeed(ig.game.units[ig.game.activeUnit].stat.spd,
                                                                            ig.game.units[ig.game.activeUnit].stat.wt, ig.game.units[ig.game.activeUnit].selectedWeapon.wt);

                                                                        console.log("Attack speed of unit is: " + ig.game.units[ig.game.activeUnit].derived_stats.attack_speed);

                                                                        // Compute Evade rate of active unit
                                                                        ig.game.units[ig.game.activeUnit].mod.evade = ig.game.computeEvade(ig.game.units[ig.game.activeUnit].derived_stats.attack_speed,
                                                                            ig.game.units[ig.game.activeUnit].stat.luk, 20);

                                                                        console.log("The evade rate is: " + ig.game.units[ig.game.activeUnit].mod.evade);

                                                                        // Computes the base hit rate using unit's basic stats
                                                                        ig.game.units[ig.game.activeUnit].stat.hit = ig.game.computeHitRate(ig.game.units[ig.game.activeUnit].stat.skl,
                                                                            ig.game.units[ig.game.activeUnit].stat.luk);



                                                                        console.log("The base hit rate is: " + ig.game.units[ig.game.activeUnit].stat.hit);
                                                                        ...
                                                                        console.log("Atk stat after equipping the " + ig.game.units[ig.game.activeUnit].selectedWeapon.name + " is: " + ig.game.units[ig.game.activeUnit].derived_stats.atk);
                                                                        console.log("Wt stat after equipping the " + ig.game.units[ig.game.activeUnit].selectedWeapon.name + " is: " + ig.game.units[ig.game.activeUnit].derived_stats.weight);
                                                                        console.log("Hit stat after equipping the " + ig.game.units[ig.game.activeUnit].selectedWeapon.name + " is: " + ig.game.units[ig.game.activeUnit].derived_stats.hit_rate);
                                                                        console.log("Crit stat after equipping the " + ig.game.units[ig.game.activeUnit].selectedWeapon.name + " is: " + ig.game.units[ig.game.activeUnit].derived_stats.crit_rate);
                                                                        ...
                                                                        console.log("total attack: " + ig.game.units[ig.game.activeUnit].derived_stats.atk);

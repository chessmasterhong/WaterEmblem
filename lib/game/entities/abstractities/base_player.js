...
.defines(function() {
    ...
    ig.global.EntityBase_player = ig.Entity.extend({
        ...
        // Level up bonuses
        // Used to determine where to draw bonuses
        levelUpBonuses: {
            hp:  0,  // Hit Points
            str: 0,  // Strength
            mag: 0,  // Magic
        ...
        // Did we already apply the terrain mod?
        appliedTerrainMod: false,
        ...
        // Enumeration of items that can be equipped: Swords, Lances, Axes, Spells, Staffs
        // This is specific to each unit classType
        validWeapon: [null],
        ...
        hasEquippedItem: false, // Does the unit have an equipped item?
                    ...
                    if(this.idleTimer.delta() > 0.2) {
                        ...
                        if(this.mov_curr > 0) {
                            if(this.destination === null) {
                                // Wait for user's input and check if the selected tile doesn't have a unit on it
                        ...
                        // If unit used up all their movement points, end their turn
                            ...
                            if(typeof ig.game.getEntityByName('btn_wait') === 'undefined' && !ig.game.menuVisible) {
                                ...
                                var i, ent, detected_player_unit, detected_enemy_unit;
                                ...
                                if(this.item[0] !== null) {
                                    if(this.item[0].affinity !== 'staff') {
                                        ...
                                        for(i = 0; i < attack_tile_search.length; i++) {
                                            ...
                                            detected_enemy_unit = ent.checkEntities();
                                        ...
                                        for(i = 0; i < assist_tile_search.length; i++) {
                                            // Spawn dummy tile entities at nearby tiles and run each of their checkEntities() methods.
                                            ent = ig.game.spawnEntity(ig.global.EntityTile_dummy, assist_tile_search[i].pos_x + ig.global.tilesize / 2, assist_tile_search[i].pos_y + ig.global.tilesize / 2);

                                            // For those that returns a unit that are enemies, push that unit to potential attack targets array.
                                            detected_player_unit = ent.checkEntities();
                                ...
                                for(i = 0; i < trade_tile_search.length; i++) {
                                    // Spawn dummy tile entities at nearby tiles and run each of their checkEntities() methods.
                                    ent = ig.game.spawnEntity(ig.global.EntityTile_dummy, trade_tile_search[i].pos_x + ig.global.tilesize / 2, trade_tile_search[i].pos_y + ig.global.tilesize / 2);
                                ...
                                // Begin menus ---------------------------------
                                if(ig.game.item_drop === null && !ig.game.units[ig.game.activeUnit].leveledUp && !ig.game.units[ig.game.activeUnit].gainedExp) {
                                    // At least one enemy is within targetable range, display attack button
                                    if(ig.game.attackTargets.length > 0 && ig.game.units[ig.game.activeUnit].hasEquippedItem && ig.game.units[ig.game.activeUnit].item[0].affinity !== 'staff' && ig.game.isValidWeapon(ig.game.units[ig.game.activeUnit].item[0].affinity, ig.game.units[ig.game.activeUnit])) {
                                        ig.game.spawnEntity(ig.global.EntityButton_attack, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Attack button
                                    }
                                    ...
                                    if(ig.game.assistTargets.length > 0) {
                                        ...
                                        for(var t = 0; t < ig.game.assistTargets.length; t++) {
                                            if(ig.game.assistTargets[t].health < ig.game.assistTargets[t].health_max) {
                                                ig.game.spawnEntity(ig.global.EntityButton_heal, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Heal button
                                                break;
                                            }
                                        }
                                    }
                                    ...
                                    if(ig.game.tradeTargets.length > 0) {
                                        ig.game.spawnEntity(ig.global.EntityButton_trade, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Trade button
                                    }

                                    if(ig.game.getTerrain(this.pos.x, this.pos.y).type === 'Armory') {
                                        ig.game.spawnEntity(ig.global.EntityButton_armory, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Armory Button
                                    }

                                    ig.game.spawnEntity(ig.global.EntityButton_inventory, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Inventory button

                                    // Only spawn wait button if this unit is not stacking on top of any other unit
                                    var ent = ig.game.spawnEntity(ig.global.EntityTile_dummy, this.pos.x + ig.global.tilesize / 2, this.pos.y + ig.global.tilesize / 2);
                                    if(!ent.checkEntities(this)) {
                                        ig.game.spawnEntity(ig.global.EntityButton_wait, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Wait button
                                    }

                                    if(ig.game.getTerrain(this.pos.x, this.pos.y).type === 'Throne' && ig.game.levels[ig.global.currentLevel].description === 'Seize' && ig.game.units[ig.game.activeUnit] instanceof ig.global.EntityHero) {
                                        ig.game.spawnEntity(ig.global.EntityButton_seize, ig.game.screen.x + 32, ig.game.screen.y + (++menuButtonIndex * 32)); // Seize buttong
                                    }
                                }
                                // End menus -----------------------------------
                            }
        ...
        unitsTileSearch: function(attack_distance, attack_area) {
            ...
            for(i = 1; i <= attack_area; i++) {
                for(j = 0; j <= attack_distance + i - 1; j++) {
                    unitTiles.push({
        ...
        levelUpStats: function() {
            if(this.levelUpStatPercentage.hp > Math.random()) { this.health_max++; this.levelUpBonuses.hp++; }
            if(this.levelUpStatPercentage.mag > Math.random()) { this.stat.mag++; this.levelUpBonuses.mag++; }
            if(this.levelUpStatPercentage.atk > Math.random()) { this.stat.str++; this.levelUpBonuses.str++; }
            if(this.levelUpStatPercentage.def > Math.random()) { this.stat.def++; this.levelUpBonuses.def++; }
            if(this.levelUpStatPercentage.skl > Math.random()) { this.stat.skl++; this.levelUpBonuses.skl++; }
            if(this.levelUpStatPercentage.res > Math.random()) { this.stat.res++; this.levelUpBonuses.res++; }
            if(this.levelUpStatPercentage.spd > Math.random()) { this.stat.spd++; this.levelUpBonuses.spd++; }
            if(this.levelUpStatPercentage.luk > Math.random()) { this.stat.luk++; this.levelUpBonuses.luk++; }
        },

        leftClicked: function() {
            if(ig.game.battleState !== 'trading' && this._killed === false && ig.game.battleState !== 'shopping' && ig.game.battleState !== 'buying' && ig.game.units[ig.game.activeUnit].unitType !== 'enemy' && ig.game.battleState !== 'battle_transitionIn' && ig.game.battleState !== 'battle_prepare' && ig.game.battleState !== 'battle_transitionOut' && ig.game.battleState !== 'battle_start' && ig.game.battleState !== 'selling' && ig.game.battleState !== 'stats' && ig.game.battleState !== 'discarding' && ig.game.battleState !== 'objective') {
        ...
        check: function(other) {
            ...
            if(other instanceof ig.global.EntityBase_enemy) {
        ...
        draw: function() {
            ...
            if((typeof ig.game.units !== 'undefined' && ig.game.units[ig.game.activeUnit] === this && !this.turnUsed && this.idleTimer.delta() > 0.2) && ig.game.displayedPlayerPhaseModal === true) {
                var i, j, radius, range,
                    ctx = ig.system.context;

                // Available movement tiles
                if(this.mov_curr > 0) {
                    radius = this.stat.mov + 1;
                    range = this.atk_dist + this.atk_area - 1;
                ...
                // Available attack tiles
                    ...
                    radius = ig.game.battleState === 'trade' ? 1 : this.atk_dist;
                    range = ig.game.battleState === 'trade' ? 1 : this.atk_area;
        ...
        receiveDamage: function(amount, from) {
            ...
            if(
                (from instanceof ig.global.EntityBase_enemy  && amount < 0) || // Prevent enemy units from healing player units
                (from instanceof ig.global.EntityBase_player && amount > 0)    // Prevent friendly fire (?)
            ) {
                amount = 0;
            }

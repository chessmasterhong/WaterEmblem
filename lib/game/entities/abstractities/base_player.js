...
.defines(function() {
    ...
    ig.global.EntityBase_player = ig.Entity.extend({
        ...
        //Determine which stats increase
        levelUpPercentage: {atk:40, mag:10, def:40, res:40, spd:70, luk:80},
                        ...
                        if(this.mov_curr < this.stat.mov) {
                            if(this.destination === null) {
                                ...
                                if(ig.input.state('up') || ig.input.state('down') || ig.input.state('left') || ig.input.state('right')) {
                                    ...
                                    if(this.mov_curr + ig.game.terrain[this.tile_index].move > this.stat.mov) {
        ...
        levelStats: function(){
            //levelUpPercentage: {atk:40, mag:10, def:40, res:40, spd:70, luk:80},
            //magic stats
            if(this.levelUpPercentage.mag >this.randomN()){ this.stat.mag+=1;}

            //attack stats
            if(this.levelUpPercentage.atk >this.randomN()){ this.stat.atk+=1;}

            //defense stats
            if(this.levelUpPercentage.def >this.randomN()){ this.stat.def+=1;}

            //resistance stats
            if(this.levelUpPercentage.res >this.randomN()){ this.stat.res+=1;}

            //speed stats
            if(this.levelUpPercentage.spd >this.randomN()){ this.stat.spd+=1;}

            //luck stats
            if(this.levelUpPercentage.luk >this.randomN()){ this.stat.luk+=1;}
        ...
        draw: function() {
            ...
            // Begin available movement tiles ----------------------------------

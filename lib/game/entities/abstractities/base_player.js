        ...
        unitsTIleSearch: function(attack_distance, attack_area) {
            ...
            for(var i = 1; i <= attack_area; i++) {
                for(var j = 0; j <= attack_distance + i - 1; j++) {
                    unitTiles.push({pos_x: this.pos.x - (attack_distance - j + i - 1) * ig.global.tilesize, pos_y: this.pos.y - j * ig.global.tilesize});
            ...
            for(var i = 1; i <= attack_area; i++) {
                for(var j = 1; j <= this.atk_dist + i - 1; j++) {
                    unitTiles.push({pos_x: this.pos.x + (attack_distance - j + i) * ig.global.tilesize, pos_y: this.pos.y - (j - 1) * ig.global.tilesize});

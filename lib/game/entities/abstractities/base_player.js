...
.requires(
    ...
    'game.entities.menus.button_item_use',
                        ...
                        // If unit used up all their movement points, end their turn
                            ...
                            if(typeof ig.game.getEntityByName('btn_wait') === 'undefined' && !ig.game.menuVisible) {
                                ...
                                if(ig.game.attackTargets.length > 0) {
                                    // Begin Attack Button ---------------------
                                    ig.game.spawnEntity(EntityButton_attack, ig.game.screen.x + 32, ig.game.screen.y + ++menuButtonIndex * 32);
                                    // End Attack Button -----------------------
                                }

                                // At least one enemy is within targetable range, display attack button
                                if(ig.game.assistTargets.length > 0) {
                                    // If unit is equipped with a staff, allow healing
                                    if(this.item[0] !== null && this.item[0].affinity === 'staff') {
                                        // Begin Heal Button -----------------------
                                        ig.game.spawnEntity(EntityButton_heal, ig.game.screen.x + 32, ig.game.screen.y + ++menuButtonIndex * 32);
                                        // End Heal Button -------------------------
                                    }

                                    // Begin Trade Button ----------------------
                                    ig.game.spawnEntity(EntityButton_trade, ig.game.screen.x + 32, ig.game.screen.y + ++menuButtonIndex * 32);
                                    // End Trade Button ------------------------
                                }
                                    ...
                                    pressedUp: function() {
                                        ...
                                        if(typeof ig.game.getEntityByName('btn_item_0') === 'undefined') {
                                            ...
                                            for(var i = 0; i < ig.game.units[ig.game.activeUnit].item.length; i++) {
                                                ...
                                                ig.game.spawnEntity(Button, ig.game.screen.x + 32, ig.game.screen.y + (i + 1) * 32, {
                                                    name: 'btn_item_' + i,
                                                    index: i,
                                                    ...
                                                    pressedUp: function() {
                                                        ...
                                                        if(typeof ig.game.getEntityByName('btn_item_use') !== 'undefined')
                                                            ig.game.getEntityByName('btn_item_use').kill();
                                                        if(typeof ig.game.getEntityByName('btn_item_discard') !== 'undefined')
                                                            ig.game.getEntityByName('btn_item_discard').kill();
                                                        ...
                                                        if(ig.game.units[ig.game.activeUnit].item[ig.game.units[ig.game.activeUnit].selectedItemIndex] !== null) {
                                                            ...
                                                            if(ig.game.units[ig.game.activeUnit].selectedItemIndex !== 0) {
                                                                if(ig.game.units[ig.game.activeUnit].item[ig.game.units[ig.game.activeUnit].selectedItemIndex].type === 'equip') {
                                                                    ...
                                                                    // Check if equip button exists. If not spawn it, else do nothing.
                                                                    if(typeof ig.game.getEntityByName('btn_item_use') === 'undefined') {
                                                                        // Spawn the use button.
                                                                        ig.game.spawnEntity(EntityButton_item_use, ig.game.screen.x + 296, ig.game.screen.y + 32);
                                                                    }
                                                                }
                                                            // The selected item was in the unit's inventory slot 0 (equipped slot), unequip the item
                                                                ...
                                                                // Check if equip button exists. If not spawn it, else do nothing.
                                                                if(typeof ig.game.getEntityByName('btn_weapon_equip') === 'undefined') {
                                                                    // Spawn the unequip button.
                                                                    ig.game.spawnEntity(Button, ig.game.screen.x + 296, ig.game.screen.y + 32, {
                                                                        ...
                                                                        pressedUp: function() {
                                                                            ...
                                                                            var slot = ig.game.freeSlot(ig.game.units[ig.game.activeUnit])
                                                                            if(slot !== null) {
                                                            ...
                                                            if(typeof ig.game.getEntityByName('btn_item_discard') === 'undefined') {
                                                                // Spawn the discard button.
                                                                ig.game.spawnEntity(Button, ig.game.screen.x + 296, ig.game.screen.y + 96, {
                                                                    name: 'btn_item_discard',
                                                                    text: ['Discard'],

                                                                    // Discard button pressedUp()
                                                                    pressedUp: function() {
                                                                        // Discard the item (delete the item from inventory)
                                                                        // TODO: Prompt user for confirmation before discarding item
                                                                        console.log('Discarded ' + ig.game.units[ig.game.activeUnit].item[ig.game.units[ig.game.activeUnit].selectedItemIndex] + '.');
                                ...
                                // Begin wait button ---------------------------
                                ig.game.spawnEntity(EntityButton_wait, ig.game.screen.x + 32, ig.game.screen.y + ++menuButtonIndex * 32);
                                // End wait button -----------------------------
                            }
                        }
                    }
        ...
        draw: function() {
            ...
            if((typeof ig.game.units !== 'undefined' && ig.game.units[ig.game.activeUnit] === this && !this.turnUsed && this.idleTimer.delta() > 0.2) && ig.game.clickedUnit === this) {
                ...
                // Available attack tiles
                    ...
                    var ctx = ig.system.context;
                    var radius = ig.game.attackTargets.length > 0 || ig.game.battleState === 'attack' ? this.atk_dist + 1 : ig.game.assistTargets.length > 0 ? 2 : 0;

                    for(var i = 0; i < radius; i++) {
                        ctx.save();
                            ctx.fillStyle = ig.game.attackTargets.length > 0 || ig.game.battleState === 'attack' ? 'rgba(255, 0, 0, 0.25)' : ig.game.assistTargets.length > 0 ? 'rgba(0, 127, 0, 0.25)' : 'rgba(0, 0, 0, 0)';

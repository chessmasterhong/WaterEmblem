...
.defines(function() {
    ...
    ig.global.EntityBase_player = ig.Entity.extend({
        ...
        exp: 0, //character exp starts at 0
        levelup: 300, // level up at constant rate of 300 exp
        ...
        stat: {
            ...
            spd: 1, // Speed
            luk: 1, // Luck
            mov: 3, // Movement points (radius in tiles)
        },
        ...
        check: function(other) {
            ...
            if(other instanceof EntityBase_enemy) {
                ...
                if(ig.game.units[ig.game.activeUnit] === this) {
                    ...
                    ig.game.units[ig.game.activeUnit].exp +=other.exp;
                    if(this.exp==this.levelup)
                    {
                        this.levelUpStats();
                        this.level++;
                        this.levelup+=300;
                        if(this.level<20) { this.exp=50;}
                        else{ this.exp=0;}
                    }

                    console.log(this.name + " EXP:" + this.exp +" ; LEVEL:"+this.level +". STATS:");

                    ...
                    if(this.idleTimer.delta() > 0.2) {
                        ...
                        // If unit used up all their movement points, end their turn
                            ...
                            if(typeof ig.game.getEntityByName('btn_wait') === 'undefined' && !ig.game.menuVisible) {
                                ...
                                for(var i = 1; i <= this.atk_area; i++) {
                                    for(var j = 0; j <= this.atk_dist + i - 1; j++) {
                                        attack_tile_search.push({pos_x: this.pos.x - (this.atk_dist - j + i - 1) * ig.global.tilesize, pos_y: this.pos.y - j * ig.global.tilesize});

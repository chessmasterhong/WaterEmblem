        ...
        draw: function() {
            ...
            if((typeof ig.game.units !== 'undefined' && ig.game.units[ig.game.activeUnit] === this && !this.turnUsed && this.idleTimer.delta() > 0.2) && ig.game.clickedUnit === this) {
                ...
                // Available attack tiles
                    ...
                    var ctx = ig.system.context;
                    var radius = ig.game.attackTargets.length > 0 || ig.game.battleAnimStage === 'attack' ? this.atk_dist + 1 : ig.game.assistTargets.length > 0 ? 2 : 0;

                    for(var i = 0; i < radius; i++) {
                        ctx.save();
                            ctx.fillStyle = ig.game.attackTargets.length > 0 || ig.game.battleAnimStage === 'attack' ? 'rgba(255, 0, 0, 0.25)' : ig.game.assistTargets.length > 0 ? 'rgba(0, 127, 0, 0.25)' : 'rgba(0, 0, 0, 0)';

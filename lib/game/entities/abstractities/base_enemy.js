    ...
    ig.global.EntityBase_enemy = ig.Entity.extend({
        ...
        selectBestTarget: function(targetable_units) {
            ...
            if(targetable_units !== null) {
                ...
                console.log(this.name + ': Finding in-range units.');
                ...
                if(player_inRange.length > 0) {
                    console.log(this.name + ': Finding in-range, lowest health units.');
                    ...
                    if(player_inRange_minHealth.length === 1) {
                        return player_inRange_minHealth[0];
                    // If more than one unit is in-range and has minimum health --> target unit that does the most damage
                        ...
                        // If main character is one of the units in range with minimum health --> target main character
                        for(var p = 0; p < player_inRange_minHealth.length; p++) {
                            if(player_inRange_minHealth[p].name === 'player_main')
                                return player_inRange_minHealth[p];
                        }

                        // If main character not found, sort unit order by descending damage and select maximum value
                        console.log(this.name + ': Finding in-range, lowest health, highest damage units.');
                    ...
                    if(player_inRange_minHealth_maxDamage.length === 1) {
                        return player_inRange_minHealth_maxDamage[0];
                    // If more than one unit is in-range, has minimum health, and has maximum damage --> target = closet unit
                        ...
                        // If main character is one of the units in range with minimum health and maximum damage --> target main character
                        for(var p = 0; p < player_inRange_minHealth_maxDamage.length; p++) {
                            if(player_inRange_minHealth_maxDamage[p].name === 'player_main')
                                return player_inRange_minHealth_maxDamage[p];
                        }

                        // If main character not found, find closet unit
                        console.log(this.name + ': Finding in-range, lowest health, highest damage, closest distance units.');
                        for(var p = 0; p < player_inRange_minHealth_maxDamage.length; p++) {
                    ...
                    if(player_inRange_minHealth_maxDamage.length === 1) {
                        ...
                        // If only one unit is in-range, has minimum health, has maximum damage, and minimum distance --> target = that unit
                        if(player_inRange_minHealth_maxDamage_minDistance.length === 1) {
                            return player_inRange_minHealth_maxDamage_minDistance[0];
                        // If more than one unit is in-range, minimum health, has maximum damage, and minimum distance
                        } else if(player_inRange_minHealth_maxDamage_minDistance.length > 1) {
                            // If main character is one of the units in range with minimum health, maximum damage, minimum distance --> target main character
                            for(var p = 0; p < player_inRange_minHealth_maxDamage_minDistance.length; p++) {
                                if(player_inRange_minHealth_maxDamage_minDistance[p].name === 'player_main')
                                    return player_inRange_minHealth_maxDamage_minDistance[p];
                            }

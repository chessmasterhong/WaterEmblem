            ...
            if(ig.game.units[ig.game.activeUnit].name === this.name) {
                ...
                if(!this.turnUsed) {
                    ...
                    if(this.destination === null) {
                        ...
                        // Determine which pathfinding array index to use (if unit has to make a turn around corner or not)
                        if(typeof this.path[1] !== 'undefined')
                            this.destination = ig.global.alignToGrid(this.path[1].x, this.path[1].y);
                        else
                            this.destination = ig.global.alignToGrid(this.path[0].x, this.path[0].y);
                    }

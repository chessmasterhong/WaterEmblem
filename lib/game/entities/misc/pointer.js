    ...
    ig.global.EntityPointer = ig.Entity.extend({
        ...
        update: function() {
            ...
            if(ig.game.menuVisible) {
                // Free-moving mouse cursor when displaying menu (so cursor does not "avoid" buttons by snapping to grid)
                this.pos = {
                    x: ig.input.mouse.x + ig.game.screen.x,
                    y: ig.input.mouse.y + ig.game.screen.y
                };
            } else {
                // Grid-snapping mouse cursor when not displaying menu (so user can see which tile is being hovered over)

 ...
 *  Director Plugin
 *  -----
 *  Original Author: Mike Laidlaw (boneheadmed)
 *  Original Source: https://github.com/boneheadmed/Director
 *
 *  A Director plugin which is really simply a way to manage and move between
 *  levels. It stores an array of levels and allows one to for example advance
 *  to the next level, go back one level, go to last level, append new levels,
 *  etc.
 */
...
.defines(function() {
    ig.Director = ig.Class.extend({
        ...
        append: function(levels) {
            ...
            newLevels = [];
            if(typeof(levels) === 'object' && typeof(levels[0].data) === 'object') {
                if(levels.constructor === (new Array).constructor) {
                    for(var i = 0; i < levels.length; i++) {
                        newLevels.push(levels[i].data);
                    }
                }
                else {
                    newLevels[0] = levels[0].data;
                }

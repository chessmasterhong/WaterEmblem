...
ig.Entity.inject({
    ...
    _retracePathToValidEnd: function(invalidEntityTypes) {
        if (this.path) {
            ...
            while (this.invalidEnd(lastWayPoint, invalidEntityTypes)) {
                ...
                if (lastWayPoint.x == this.path[this.path.length-1].x && lastWayPoint.y == this.path[this.path.length-1].y) {
    ...
    _createNewLimitedPath: function () {
        var newPath = [];
        var distance = 0;
        var maxMovement = this.maxMovement;
            ...
            for (var i = 0; i < this.path.length; i++) {
                ...
                if (this.path[i]) {
                    ...
                    if (newDistance > maxMovement) {
                        ...
                        var newWayPointLength = maxMovement - distance;
